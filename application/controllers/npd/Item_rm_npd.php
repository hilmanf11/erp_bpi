...
class Item_rm_npd extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        $this->form_validation->set_rules('number', 'Product No.', 'required|min_length[1]|max_length[100]|is_unique[item_rm_npd.number]');
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('npd/item_rm_npd');
        } else {
    ...
    public function reads()
    {
        ...
        $send = $this->crud->query("SELECT a.*, b.name as item_family_name FROM item_rm_npd a JOIN item_familys b ON a.item_family_id = b.id WHERE a.number like '%$post%' or a.name like '$post'");
    ...
    {
        $post = isset($_POST['q']) ? $_POST['q'] : "";
        $send = $this->crud->query("SELECT a.*, b.name as item_family_name FROM item_rm_npd a JOIN item_familys b ON a.item_family_id = b.id WHERE a.item_family_id like '%$id%'");
        echo json_encode($send);
    }
    ...
    public function autoid($category, $family)
    {
        ...
        $sql = $this->db->query("SELECT max(id) as kode FROM item_rm_npd WHERE id LIKE '%$format%'");
        $row = $sql->row();
        if ($row->kode == "") {
            $kode = 0;
        } else {
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $send   = $this->crud->create('item_rm_npd', $post);
                echo $send;
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $send = $this->crud->update('item_rm_npd', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('item_rm_npd', $data);
        echo $send;
    }
    ...
    {
        @unlink('failed/item_rm_npd.txt');
    }
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            $message = $this->input->post('message');
            $textFailed = fopen('failed/item_rm_npd.txt', 'a');
    ...
    public function uploadCreate()
    {
        ...
        try {
            ...
            $item_rm_npd = $this->crud->read('item_rm_npd', [], ["number" => $data['number']]);
            ...
            if (strtolower($data['action']) !== 'update' && !empty($item_rm_npd)) {
            ...
            if (strtolower($data['action']) === 'update') {
                if (empty($item_rm_npd)) {
                    ...
                    $send = $this->crud->update('item_rm_npd', ["id" => $item_rm_npd->id], $dataFinal);
                }
                echo $send;
                ...
                $dataFinal['id'] = $autoid;
                $send = $this->crud->create('item_rm_npd', $dataFinal);
                echo $send;
            }
    ...
    public function uploadcreate_backup()
    {
        if ($this->input->post()) {
            ...
            } elseif (!empty($item_rm_npd->number)) {
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=item_rm_npd_$format.xls");
        ...
        $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#item_rm_npd {border-collapse: collapse;width: 100%;font-size: 12px;}#item_rm_npd td, #item_rm_npd th {border: 1px solid #ddd;padding: 2px;}#item_rm_npd tr:nth-child(even){background-color: #f2f2f2;}#item_rm_npd tr:hover {background-color: #ddd;}#item_rm_npd th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
            <center>
