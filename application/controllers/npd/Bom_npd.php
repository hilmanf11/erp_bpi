...
class bom_npd extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        $this->form_validation->set_rules('item_fg_id', 'Product No.', 'required|min_length[1]|max_length[20]|is_unique[bom_npd.item_fg_id]');
        $this->form_validation->set_rules('item_rm_id', 'Part No.', 'required|min_length[1]|max_length[20]|is_unique[bom_npd.item_rm_id]');
    }
    //HALAMAN UTAMA
    ...
    public function reads()
    {
        ...
        $send = $this->crud->reads('bom_npd', ["item_fg_id" => $post]);
        echo json_encode($send);
    }

    public function readItem()//belum ke npd karena masih kosong
    {
        $post = isset($_POST['q']) ? $_POST['q'] : "";
        ...
        $item_fg_npd = $this->crud->query("SELECT * FROM item_fg_npd WHERE `type` = 'SA' AND status = 0 AND (number like '%$post%' or name like '$post')");
     ...
     public function readWeight()
     {
        ...
        $item_fg_npd = $this->crud->read("item_fg_npd", [] ,["id" => $post['item_fg_id']]);
        echo json_encode($item_fg_npd);
    }
   ...
   public function readRunner()
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $this->db->join('item_fg_npd b', 'a.item_fg_id = b.id');
    ...
    public function datatableDetails()
    {
        if ($this->input->get()) {
            ...
            $this->db->from('bom_npd a');
            $this->db->join('item_fg_npd b', 'a.item_fg_id = b.id','left');
            $this->db->join('item_rm c', 'a.item_rm_id = c.id', 'left');
            $this->db->join('item_familys d', 'c.item_family_id = d.id', 'left');
            $this->db->join('item_fg_npd e', 'a.item_fg_sa_id = e.id','left');
    ...
    public function create_SA()
    {
        if ($this->input->post()) {
            ...
            // $bom_npd = $this->crud->read("bom_npd", [], ["item_fg_id" => $post['item_fg_id'], "item_rm_id" => $post['item_rm_id']]);
            ...
            if (@$post['id'] != "") {
                $send = $this->crud->update('bom_npd', ["id" => $post['id']], $dataFinal);
            } else {
                $send = $this->crud->create('bom_npd', $post);
            }
            echo $send;
        } else {
    ...
    //     var_dump($data);
    //     $send = $this->crud->delete('bom_npd', $data);
    //     echo $send;
    // }
    ...
    {
        $data = $this->input->post();
        $bom_npd_item_rm_id = $this->crud->read("bom_npd", [], ["item_fg_id" => $data['item_fg_id'], "item_rm_id" => $data['item_rm_id']]);
        
        if (!empty($bom_npd_item_rm_id)) {
            $dataFinal = [
        ...
        $send = $this->crud->delete('bom_npd', $dataFinal);
        echo $send;
    }
    ...
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            ...
            $textFailed = fopen('failed/bom_npd.txt', 'a');
    ...
    public function uploadCreate()
    {
        if ($this->input->post()) {
            ...
            $item_fg_npd = $this->crud->read('item_fg_npd', [], ["id" => $data['item_fg_id']]);
            ...
            $bom_npd = $this->crud->read('bom_npd', [], ["item_fg_id" => $data['item_fg_id'], "item_rm_id" => $data['item_rm_id']]);

            if (empty($item_fg_npd->id)) {
            ...
            } elseif (!empty($bom_npd->item_rm_id)) {
                ...
                $send   = $this->crud->create('bom_npd', $dataFinal);
                echo $send;
            }
        }
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=bom_npd_$format.xls");
        }
        ...
        $this->db->order_by('a.id', 'ASC');
        $records = $this->db->get()->result_array();
        $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#bom_npd {border-collapse: collapse;width: 100%;font-size: 12px;}#bom_npd td, #bom_npd th {border: 1px solid #ddd;padding: 2px;}#bom_npd tr:nth-child(even){background-color: #f2f2f2;}#bom_npd tr:hover {background-color: #ddd;}#bom_npd th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
        <center>
