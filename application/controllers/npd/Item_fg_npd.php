...
class Item_fg_npd extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        $this->form_validation->set_rules('number', 'Product No.', 'required|min_length[1]|max_length[100]|is_unique[item_fg_npd.number]');
    }
    //HALAMAN UTAMA
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('npd/item_fg_npd');
        } else {
    ...
    public function reads()
    {
        ...
        $send = $this->crud->query("SELECT * FROM item_fg_npd WHERE status = '0' AND (number like '%$post%' or number_customer like '%$post%' or name like '%$post%' or id like '%$post%') ORDER BY number ASC");
    ...
    public function readFamily()
    {
        ...
        $this->db->where('a.item_category_id', 'C03');
        $records = $this->db->get()->result_array();
    ...
    public function delete()
    {
        ...
        $file = $this->crud->read('item_fg_npd', [], ["id" => $data['id']]);
        $send = $this->crud->delete('item_fg_npd', $data);
    ...
    {
        @unlink('failed/item_fg_npd.txt');
    }
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            $message = $this->input->post('message');
            $textFailed = fopen('failed/item_fg_npd.txt', 'a');
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $item_fg_npd = $this->crud->read('item_fg_npd', [], ["number" => $data['number']]);
            $division = $this->crud->read('divisions', [], ["id" => $data['division_id']]);
            $item_familys = $this->crud->read('item_familys', [], ["name" => $data['item_family_name']]);
            ...
            if (empty($division->number)) {
            ...
            } elseif (!empty($item_fg_npd->number)) {
                ...
                $dataFinal = array(
                    ...
                    "type" => $data['type'],
                    "item_family_id" => $item_familys->id,
                ...
                $send   = $this->crud->create('item_fg_npd', $dataFinal);
                echo $send;
            }
        }
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=item_fg_npd_$format.xls");
        }
        //Config
        $this->db->select('*');
        $this->db->from('config');
        ...
        $this->db->order_by('a.id', 'ASC');
        $records = $this->db->get()->result_array();
        $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#item_fg_npd {border-collapse: collapse;width: 100%;font-size: 12px;}#item_fg_npd td, #item_fg_npd th {border: 1px solid #ddd;padding: 2px;}#item_fg_npd tr:nth-child(even){background-color: #f2f2f2;}#item_fg_npd tr:hover {background-color: #ddd;}#item_fg_npd th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
        <center>
