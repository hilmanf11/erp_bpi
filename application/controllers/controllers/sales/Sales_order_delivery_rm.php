...
class Sales_order_delivery_rm extends CI_Controller
{
    ...
    public function create()
    {
        if ($this->input->post()) {
            ...
            if (round($qty_so) >= round(@$sales_order_delivery_rm_total[0]->total + $post['qty'])) {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $sales_orders = $this->crud->read("sales_order_rm", [], ["sales_order_no" => $sales_order_no, "item_fg_id" => $item_fg_id]);
            $sales_order_deliveries = $this->crud->read("sales_order_delivery_rm", [], ["sales_order_no" => $sales_order_no, "item_fg_id" => $item_fg_id, "trans_date" => $post['trans_date']]);
            $sales_order_deliveries_total = $this->crud->query("SELECT SUM(qty) as total FROM sales_order_delivery_rm WHERE sales_order_no='$sales_order_no' and item_fg_id = '$item_fg_id' and id != '$id' GROUP BY sales_order_no, item_fg_id");
