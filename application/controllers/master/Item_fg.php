...
class item_fg extends CI_Controller
{
    ...
    public function reads()
    {
        ...
        $send = $this->crud->query("SELECT * FROM item_fg WHERE status = '0' AND number like '%$post%' or number_customer like '%$post%' or name like '%$post%'");
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $attachment = $this->crud->upload('attachment', ["pdf"], 'assets/documents/item_fg/', ["id" => $id], "item_fg", "attachment");
            $postFinal = array_merge($post, ["attachment" => $attachment]);
