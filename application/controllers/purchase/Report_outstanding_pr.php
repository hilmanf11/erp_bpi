...

{
    ...
    public function readItems()
    {
        ...
        $this->db->select('c.*, a.id as supplier_item_id, b.currency, c.number as item_number, c.name as item_name, c.id as item_id');
        $this->db->from('item_rm c');
        $this->db->join('supplier_items a', 'c.id = a.item_rm_id', 'left');
...

    {
        ...
        $item_family_id = $this->input->get("item_family_id");
        $item_rm_id = $this->input->get("item_id");
        $post = isset($_POST['q']) ? $_POST['q'] : "";

        $sales_orders = $this->crud->query("SELECT a.request_no as `number` 
    ...
    public function print($option = "")
    {
        ...
        $this->db->select('a.*, c.name as user_name, e.po_no, e.status as po_no_status');
        ...
        $this->db->join('purchase_orders e', 'a.request_no = e.request_no', 'left');
        ...
        foreach ($records as $data) {
            ...
            $this->db->select('a.*, c.po_no, b.number as item_number, b.name as item_name, b.description as item_description, b.uom, c.status as po_no_status');
            $this->db->from('purchase_requests a');
            $this->db->join('item_rm b', 'a.item_rm_id = b.id');
            ...
            if ($data['po_no_status'] == 0) {
                $status_po = "<b style='color:green;'>OPEN</b>";
            } else {
                $status_po = "<b style='color:red;'>CLOSE</b>";
            }
