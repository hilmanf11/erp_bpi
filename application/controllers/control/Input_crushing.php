...
class Input_crushing extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('control/input_crushing');
        } else {
    ...
    public function reads()
    {
        ...
        $send = $this->crud->reads('input_crushing', ["name" => $post]);
        echo json_encode($send);
    }

    public function readDocument()
    {
        $send = $this->crud->query("SELECT DISTINCT document FROM input_crushing order by document desc");
        echo json_encode($send);
    }

    public function readWoNo()
    {
        $send = $this->crud->query("SELECT DISTINCT workorder FROM input_crushing order by workorder desc");
        echo json_encode($send);
    }
    ...
    public function input_crushing_no($trans_date)
    {
        $trans_date = base64_decode($trans_date);
        $year       = date("Y", strtotime($trans_date));
        $datenow    = date("ymd", strtotime($trans_date));
        $sqlGetID   = $this->db->query("SELECT MAX(SUBSTR(document, -4, 4)) as kode FROM input_crushing WHERE trans_date like '%$year%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
    ...
    public function datatablesTemp()
    {
        ...
        $this->db->select('b.id, b.number, b.name, b.uom');
        ...
        $this->db->where('a.wo_no',$workorder);
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $this->db->like('a.workorder', $filter_wo_no);
     ...
     public function datatableDetails()
     {
         if ($this->input->get()) {
            ...
            $this->db->select('a.*, b.number as item_number, b.name as item_name, d.number as machine_number');
            $this->db->from('input_crushing a');
            $this->db->join('item_rm b', 'a.item_rm_id = b.id');
            $this->db->join('item_fg c', 'a.item_fg_id = c.id','left');
            $this->db->join('machines d', 'a.machine_id = d.id','left');
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                // $inputCrushing = $this->crud->reads("input_crushing", [], ["item_rm_id" => $post['item_rm_id'], "workorder" => $post['workorder']]);

                // if (count($inputCrushing) > 0) {
                //     echo json_encode(array("title" => "Duplicate", "message" => "Data has been created", "theme" => "error"));
                // } else {
                    $send = $this->crud->create('input_crushing',$post);
                    echo $send;
                // }
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $itemNg = $this->crud->read("input_crushing", [], ["id" => $id]);
            ...
            $send = $this->crud->update('input_crushing', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        @$itemNg = $this->crud->reads("input_crushing", [], ["id" => $data['id']]);
        ...
        @$send = $this->crud->delete('input_crushing', ["document" => $data['document']]);
        echo $send;
    }
    ...
    {
        $data = $this->input->post();
        $send = $this->crud->delete('input_crushing', $data);
        echo $send;
    }
    ...
    {
        @unlink('failed/input_crushing.txt');
    }
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            $message = $this->input->post('message');
            $textFailed = fopen('failed/input_crushing.txt', 'a');
    ...
    {
        $file = "failed/input_crushing.txt";
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $accumulate_sh = $this->db->query("SELECT COALESCE(SUM(qty_product),0) as qty FROM input_crushing WHERE workorder = '{$data['workorder']}' AND item_fg_id = '{$item_fg->id}' AND no_urut = 1")->row();
            ...
            if (!$production_schedule || empty($production_schedule->id)) {
                ...
                foreach ($records as $row) {
                    ...
                    $send = $this->crud->create('input_crushing', $dataFinal);
                }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=input_crushing_$format.xls");
        }
        $filter_from = $this->input->get('filter_from');
        $filter_to = $this->input->get('filter_to');
        $filter_document = $this->input->get('filter_document');
        ...
        $this->db->select('a.*, b.number as item_rm_number, b.name as item_rm_name, c.number as item_fg_number, c.name as item_fg_name, d.number as machine_number');
        $this->db->from('input_crushing a');
        $this->db->join('item_rm b', 'a.item_rm_id = b.id');
        $this->db->join('item_fg c', 'a.item_fg_id = c.id','left');
        $this->db->join('machines d', 'a.machine_id = d.id','left');
