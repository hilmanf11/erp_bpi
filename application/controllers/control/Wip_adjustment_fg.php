...
class Wip_adjustment_fg extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('control/wip_adjustment_fg');
        } else {
    ...
    {
        $records = $this->crud->query("SELECT `period` FROM wip_adjustment_fg WHERE `status` = '0' GROUP BY `period`");
        echo json_encode($records);
    }

    public function readWp($period)
    {
        $records = $this->crud->query("SELECT workorder FROM wip_adjustment_fg WHERE `status` = '0' and `period` = '$period' GROUP BY workorder");
        echo json_encode($records);
    }
    ...
    public function readRequestNo($period, $workorder)
    {
        ...
        $records = $this->crud->query("SELECT request_no FROM wip_adjustment_fg WHERE status = '0' and `period` = '$period' and workorder = '$workorder' GROUP BY `request_no`");
        echo json_encode($records);
    }

    public function readRequestNos()
    {
        $records = $this->crud->query("SELECT request_no FROM wip_adjustment_fg WHERE status = '0' GROUP BY `request_no`");
        echo json_encode($records);
    }
    ...
    public function request_no($trans_date)
    {
        ...
        $sqlGetID   = $this->db->query("SELECT max(request_no) as kode FROM wip_adjustment_fg WHERE request_no like '%$datenow%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
    ...
    public function datatableUpdate($request_no){
        ...
        $this->db->from('wip_adjustment_fg a');
        ...
        if ($this->input->post()) {
            // if ($this->form_validation->run() == TRUE) {
                ...
                if ($post['qty'] == 0) {
                    ...
                    $datas = $this->crud->reads('wip_adjustment_fg', [], ["request_no" => $request_no, "item_fg_id" => $item_fg_id]);

                    if(count($datas) > 0){
                        $send = $this->crud->update('wip_adjustment_fg', ["request_no" => $request_no, "item_fg_id" => $item_fg_id], $post);
                        echo $send;
                    }else{
                        $send = $this->crud->create('wip_adjustment_fg', $post);
                        echo $send;
                    }
                }
            // } else {
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('wip_adjustment_fg', ["id" => $data['id']]);
        echo $send;
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=wip_adjustment_fg_$format.xls");
        }
        // $filter_period = $this->input->get('filter_period');
        // $filter_workorder   = $this->input->get('filter_workorder');
        $filter_request_no = $this->input->get('filter_request_no');
