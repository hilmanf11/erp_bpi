...

{
    ...
    public function readItemFg($customer_id,$customer_address_id,$division)
    {
    ...
    {
        $send = $this->crud->query("SELECT DISTINCT sales_order_no, sales_order_date FROM sales_order_rm WHERE status = 0");
        echo json_encode($send);
    }
...

    {
        ...
        $datenow    = "SORM" . $customer_id . date("ymd", strtotime(base64_decode($sales_order_date)));
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $this->db->join('(SELECT sales_order_no, COUNT(status) as total_status_close FROM sales_order_rm WHERE status = 1 GROUP BY sales_order_no) c', 'a.sales_order_no = c.sales_order_no', 'left');
            $this->db->join('(SELECT sales_order_no, COUNT(status) as total_status_open FROM sales_order_rm WHERE status = 0 GROUP BY sales_order_no) d', 'a.sales_order_no = d.sales_order_no', 'left');
    ...
    public function datatableDetails()
    {
        if ($this->input->get()) {
            ...
            $this->db->join('delivery_notes c', 'a.sales_order_no = c.sales_order_no_rm AND a.item_fg_id = c.item_fg_id', 'left');
