...
class Sales_order_closing extends CI_Controller
{
    ...
    {
        $send = $this->crud->query("SELECT DISTINCT sales_order_no FROM sales_orders WHERE deleted = '0'");
        echo json_encode($send);
    }
    ...
    {
        $send = $this->crud->query("SELECT DISTINCT customer_order_no FROM sales_orders WHERE deleted = '0'");
        echo json_encode($send);
    }
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            d.name as item_fg_name, COALESCE(c.qty,0) as delivery, 
            CASE 
                WHEN a.status = 1 THEN a.qty - COALESCE(c.qty, 0)
                ELSE 0
            END as undelivery,
            CASE 
                WHEN a.status = 1 THEN 0
                ELSE a.qty - COALESCE(c.qty, 0)
            END as outstanding");
