...
class Packing_lists extends CI_Controller
{
    ...
    public function number($trans_date, $customer_number)
    {
        $datenow    = "PL-" . $customer_number . date("Ym", strtotime(base64_decode($trans_date)));
    ...
    {
        $post = isset($_POST['q']) ? $_POST['q'] : "";
        $delivery_notes = $this->crud->query("SELECT delivery_note_no as `number`, b.number as customer_number
            FROM sales_invoices a
            JOIN customers b ON a.customer_id = b.id
            WHERE a.status = '0' and a.delivery_note_no like '%$post%' 
            GROUP BY a.delivery_note_no
            ORDER BY a.delivery_note_no ASC");
    ...
    public function print($option = "")
    {
        ...
        $this->db->select('a.*, b.number as item_number, b.name as item_name, d.name as customer_name, b.uom');
        $this->db->from('packing_lists a');
        $this->db->join('item_fg b', 'a.item_fg_id = b.id');
        $this->db->join('customers d', 'a.customer_id = d.id');
        $this->db->where('a.deleted', 0);
