...
class Shipping_orders extends CI_Controller
{
    ...
    public function getDeliveryOrders()
    {
        if ($this->input->get()) {
            ...
            $this->db->select('a.customer_order_no, a.delivery_order_no, a.delivery_order_date, a.uom, a.sales_order_no, a.trans_type, a.remarks, b.number as item_fg_number, b.name as item_fg_name, c.name as customer_name, SUM(a.qty_del) as delivery, a.created_by, a.created_date, e.shipping');
            $this->db->from('delivery_orders a');
            $this->db->join('item_fg b', 'a.item_fg_id = b.id');
            $this->db->join('customers c', 'a.customer_id = c.id');
            $this->db->join("(SELECT delivery_order_no, c.item_fg_id, COALESCE(SUM(a.qty),0) as shipping 
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->db->select("a.*");
                $this->db->from('scan_item_receipts_fg a');
                $this->db->join('wip_receipts d', 'a.checksheet_number = d.checksheet_number');
                $this->db->join('sales_orders b', 'd.item_fg_id = b.item_fg_id');
                $this->db->join('delivery_orders c', 'b.item_fg_id = c.item_fg_id and c.customer_id = b.customer_id');//ok
