...

{
    ...
    public function readItems($document)
    {
        $documents = base64_decode($document);
       
       $this->db->select('a.*,b.id as item_rm_id, b.number as item_rm_number, b.name as item_rm_name');//c.specification
       $this->db->from('scraps a');
       $this->db->join('item_rm b', 'a.item_rm_id = b.id');
       $this->db->where('a.document', $documents);
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $result = array();
            //Select Query
            $this->db->select('a.*, b.number as item_number, b.name as item_name, c.request_no as material_request');
            $this->db->from('scraps a');
            $this->db->join('item_rm b', 'a.item_rm_id = b.id');
            $this->db->join('supply_requestions c', 'a.item_rm_id = c.item_rm_id and a.wo_no = c.workorder', 'left');
    ...
    public function print($option = "")
    {
        ...
        WHERE a.item_family_id like '%$filter_family_id%' and a.id like '%$filter_item_id%'
        GROUP BY a.id
        ORDER BY a.number");
