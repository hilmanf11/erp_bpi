...
class Supply_sheets extends CI_Controller
{
    ...
    public function datatables()
    {
        ...
        b.name as item_fg_name, b.uom, g.qty_issued2, COALESCE(SUM(c.qty),0) as qty_actual, COUNT(a.status) as total_status, i.total_status_open, h.total_status_close');
        ...
        $this->db->join('(SELECT request_no, COUNT(status) as total_status_close FROM supply_sheets WHERE status = 1 GROUP BY request_no) h', 'a.request_no = h.request_no', 'left');
        $this->db->join('(SELECT request_no, COUNT(status) as total_status_open FROM supply_sheets WHERE status = 0 GROUP BY request_no) i', 'a.request_no = i.request_no', 'left');
        ...
        foreach ($records as $record) {
            ...
            // //crusher
            // $cr_query = $this->db->query("SELECT id 
            //                    FROM item_rm
            ...
            // $qty_from_cr = 0;
            // if ($cr_query->num_rows() > 0) {
            //     $cr_item_rm_id = $cr_query->row()->id;

            //     $qty_query = $this->db->select('SUM(qty) as qty_total')
            //                         ->from('issued_material_details')
            //                         ->where('item_rm_id', $cr_item_rm_id)
            //                         ->where('request_no', $record['request_no'])
            //                         ->get();

            //     if ($qty_query->num_rows() > 0) {
            //         $qty_from_cr = $qty_query->row()->qty_total;
            //     }
            // }

            // //peletizing
            // $pl_query = $this->db->query("SELECT id FROM item_rm WHERE number LIKE CONCAT('PL-', (SELECT number FROM item_rm WHERE id = '$item_rm_id'))");

            // $qty_from_pl = 0;
            // if ($pl_query->num_rows() > 0) {
            //     $pl_item_rm_id = $pl_query->row()->id;

            //     $qty_query = $this->db->select('SUM(qty) as qty_total')
            //                     ->from('issued_material_details')
            //                     ->where('item_rm_id', $pl_item_rm_id)
            //                     ->where('request_no', $record['request_no'])
            //                     ->get();

            //     if ($qty_query->num_rows() > 0) {
            //     $qty_from_pl = $qty_query->row()->qty_total;
            //     }
            // }

            // // Hitung status berdasarkan qty_req dan qty_actual + qty dari item_rm_id dengan awalan "CR-"
            // if ($record['qty_req'] == ($record['qty_actual'] + $qty_from_cr + $qty_from_pl)) {
            //     $status = '1';
            // } else {
            //     $status = '0';
            // }
    ...
    public function datatablesTemp()
    {
            ...
            CASE 
                WHEN h.share_order = '100' AND h.calculate = 'YES' THEN 
                    CASE 
                        WHEN MOD(
                            CASE 
                                WHEN d.qty_bal IS NOT NULL THEN d.qty_bal
                                WHEN f.uom_soft IS NULL THEN ROUND(e.qty * b.composition, 4)
                                ELSE ROUND(e.qty * (b.composition * f.convertion), 4)
                            END + COALESCE(e.total_purging, 0), h.mpq
                        ) = 0 THEN 
        ...
        // //purging
        // $this->db->join('supply_sheets j', "c.id = j.item_rm_id", 'left');
        // $this->db->join('production_schedules k', 'j.workorder = k.wo_no and j.item_fg_id = k.item_fg_id');

        $this->db->where('b.item_fg_id', $item_fg_id);
        // $this->db->where('h.share_order', 100);
        // $this->db->where('h.calculate', 'YES');
        ...
        for ($i = 0; $i < $page; $i++) {
            ...
            foreach ($records as $record) {
                ...
                $html .= '  <tr>
                                ...
                                <td>' . $record['uom'] . '</td>
                                <td style="text-align:right;">' . @number_format($record['qty_req'], 2) . '</td>
                                <td style="text-align:right;">' . @number_format($record['mpq'], 2) . '</td>
                                <td style="text-align:right;">' . @number_format($record['qty_issueds'], 2) . '</td>
