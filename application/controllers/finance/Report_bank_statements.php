...
class Report_bank_statements extends CI_Controller
{
    ...
    public function print($option = "") // Fixed duplicate GL Number
    {
        if ($option == "excel") {
            $format  = date("Ymd");
        ...
        */
        ...
        while ($time_start <= $time_to) {
            ...
            // Kueri baru: ambil data journal_postings dengan join ke ar_receipts atau ap_payments
            $journal_entries = $this->crud->query("
                SELECT a.*, 
                    COALESCE(b.receipt_no, c.payment_no) AS document_no,
                    COALESCE(b.description, c.note) AS doc_description,
                    b.currency AS ar_currency, c.currency AS ap_currency
                FROM journal_postings a
            ...
            foreach ($journal_entries as $journal_posting) {
                $gl_no = $journal_posting->number;

                // Jika GL No sudah pernah diproses, lewati
                if (in_array($gl_no, $processed_gl_nos)) {
                    continue;
                }

                $original_debit = (float)$journal_posting->original_debit;
                $original_credit = (float)$journal_posting->original_credit;
                $local_debit = (float)$journal_posting->local_debit;
                $local_credit = (float)$journal_posting->local_credit;
                
                // Hitung saldo
                $original_balance = ($coa_original + $ar_begin_original + $ap_begin_original + $original_debit - $original_credit);
                $local_balance = ($coa_local + $ar_begin_local + $ap_begin_local + $journal_begin_local + $local_debit - $local_credit);

                $datas[] = array(
                    "trans_date" => $trans_date,
                    "document_no" => $journal_posting->document_no,
                    "gl_no" => $gl_no,
                    "description" => $journal_posting->description . " | " . $journal_posting->doc_description,
                ...
                // Perbarui saldo untuk iterasi berikutnya
                $coa_original = 0;
                $coa_local = 0;
                $ar_begin_original = 0;
                $ar_begin_local = 0;
                $ap_begin_original = $original_balance; // Saldo terakhir menjadi saldo awal
                $ap_begin_local = $local_balance;
                $journal_begin_local = 0;

                // Tandai GL No ini sudah diproses
                $processed_gl_nos[] = $gl_no;
            }
