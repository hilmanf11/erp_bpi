...
class Report_ap extends CI_Controller
{
    ...
    public function print_detail($option = "") 
    {
        ...
        // Get Suppliers
        $this->db->select("a.*, b.account_number, c.account_name, (CASE WHEN '$currency' = 'INR' THEN b.balance_local ELSE b.balance END) as balance_local");
        ...
        foreach ($suppliers as $supplier) 
        {
            ...
            if ($supplier['currency'] == $filter_currency || $filter_currency == "") {
    ...
    public function print_modified($option = "") // Bug query karena hanya tampil AP Payment sementara PI tidak tampil (Bu Nina)
    {
        if ($option == "excel") {
            $format  = date("Ymd");
            header("Content-type: application/vnd-ms-excel");
        ...
        $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#customers {border-collapse: collapse;width: 100%;font-size: 12px;}#customers td, #customers th {border: 1px solid #ddd;padding: 2px;}#customers tr:hover {background-color: #ddd;}#customers th {padding-top: 2px;padding-bottom: 2px;text-align: center;color: black;}</style><body>
            ...
            foreach ($suppliers as $supplier) 
            {
                ...
                // Bagian 1: Query dari purchase_invoices
                ...
                // Bagian 2: Query dari ap_payments
                ...
                ");
                $purchase_invoices = $final_query->result_array();
                ...
                if(count($purchase_invoices) > 0){
                    $detail .= '<tr style="background: #DEE2FF; font-weight:bold;">
                                    ...
                                    <td style="text-align:right;">'.$this->formatNominal(@$begin_balance, 2, $option).'</td>
                                    <td colspan="3"></td>
                                    <td style="text-align:right;">'.$this->formatNominal(@$begin_balance_local, 2, $option).'</td>
                                </tr>';
                }
                ...
                {                    
                    ...
                    if((@$begin_balance - $original_debit2 + $original_credit2) >= 0){
                        $balance_original = $this->formatNominal(@$begin_balance - $original_debit2 + $original_credit2, 2, $option);
                    }else{
                        $balance_original = "<span style='color:red;'>(".$this->formatNominal(abs(@$begin_balance - $original_debit2 + $original_credit2), 2, $option).")</span>";
                    }

                    if((@$begin_balance_local - $purchase_invoice['local_debit'] + $purchase_invoice['local_credit']) >= 0){
                        $balance_local = $this->formatNominal(@$begin_balance_local - $purchase_invoice['local_debit'] + $purchase_invoice['local_credit'], 2, $option);
                    }else{
                        $balance_local = "<span style='color:red;'>(".$this->formatNominal(abs(@$begin_balance_local - $purchase_invoice['local_debit'] + $purchase_invoice['local_credit']), 2, $option).")</span>";
                    }
                    ...
                    $detail .= '  <tr>
                                    ...
                                    <td style="text-align:right;">'.$this->formatNominal($original_debit2, 2, $option).'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($original_credit2, 2, $option).'</td>
                                    <td style="text-align:right;">'.$balance_original.'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($purchase_invoice['rates'], 2, $option).'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($purchase_invoice['local_debit'], 2, $option).'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($purchase_invoice['local_credit'], 2, $option).'</td>
                ...
                if(count($purchase_invoices) > 0){
                    if($begin_balance >= 0){
                        $balance_original = $this->formatNominal($begin_balance, 2, $option);
                    }else{
                        $balance_original = "<span style='color:red;'>(".$this->formatNominal(abs($begin_balance), 2, $option).")</span>";
                    }

                    if($begin_balance_local >= 0){
                        $balance_local = $this->formatNominal(@$begin_balance_local, 2, $option);
                    }else{
                        $balance_local = "<span style='color:red;'>(".$this->formatNominal(abs(@$begin_balance_local), 2, $option).")</span>";
                    }

                    $detail .= '  <tr style="background: #E5E5E5; font-weight:bold;">
                                    <td colspan="9">SUB TOTAL</td>
                                    <td style="text-align:right;">'.$this->formatNominal($original_debit, 2, $option).'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($original_credit, 2, $option).'</td>
                                    <td style="text-align:right;">'.$balance_original.'</td>
                                    <td></td>
                                    <td style="text-align:right;">'.$this->formatNominal($local_debit, 2, $option).'</td>
                                    <td style="text-align:right;">'.$this->formatNominal($local_credit, 2, $option).'</td>
            ...
            $detail .= '  <tr style="background: #C3FFB4; font-weight:bold;">
                            ...
                            <td style="text-align:right;">'. $this->formatNominal($grand_original_debit, 2, $option).'</td>
                            <td style="text-align:right;">'. $this->formatNominal($grand_original_credit, 2, $option).'</td>
                            <td style="text-align:right;">'. $this->formatNominal($grand_original_balance, 2, $option).'</td>
                            <td></td>
                            <td style="text-align:right;">'. $this->formatNominal($grand_local_debit, 2, $option).'</td>
                            <td style="text-align:right;">'. $this->formatNominal($grand_local_credit, 2, $option).'</td>
                            <td style="text-align:right;">'. $this->formatNominal($grand_local_balance, 2, $option).'</td>
                        </tr>';
