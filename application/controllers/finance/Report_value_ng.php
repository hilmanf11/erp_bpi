...
class Report_value_ng extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('finance/report_value_ng');
        } else {
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=report_value_ng_$format.xls");
        }
        ...
        $query_main = "SELECT a.id,
                ...
                LEFT JOIN standard_price_fg e ON a.id = e.item_fg_id
                WHERE a.type != 'RM' 
        ...
        foreach ($records as $record) {
            ...
            $amountTotalProduction = $total * $record->price;
            ...
            $totalAmountTotalProduction += $total * $record->price;
            $totalAmountOutputProduction += ($record->qty_actual + $record->qty_wip) * $record->price;
            $totalAmountNG += $record->qty_ng * $record->price;
            ...
            $html .= '  <tr>
                            ...
                            <td style="text-align:right;">' . number_format($amountTotalProduction, 2) . '</td>
                            <td style="text-align:right;">' . number_format(($record->qty_actual + $record->qty_wip) * $record->price, 2) . '</td>
                            <td style="text-align:right;">' . number_format($record->qty_ng * $record->price, 2) . '</td>
                        </tr>';
    ...
    public function print_sum_customer($option = "")
    {
        if ($option == "excel") {
            $format  = date("Ymd");
            header("Content-type: application/vnd-ms-excel");
            header("Content-Disposition: attachment; filename=report_value_ng_summary_customer_$format.xls");
        }
        $filter_from = $this->input->get('filter_from');
        $filter_to   = $this->input->get('filter_to');
        $filter_items = $this->input->get('filter_items');
