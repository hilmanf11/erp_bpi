...
class Report_value_ng_rm extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('finance/report_value_ng_rm');
        } else {
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=report_ng_rm_$format.xls");
        }
        ...
        //             COALESCE(c.qty_actual,0) + COALESCE(c2.qty_wip,0) + COALESCE(d.qty_ng,0) as qty_total,
        //             COALESCE(c.qty_actual,0) + COALESCE(c2.qty_wip,0) as qty_output
        //         FROM item_fg a
        ...
        $query_main = "SELECT 
            ...
            rm.name   AS `name`,
            rm.uom    AS uom,
            fam.name  AS item_family_name,
            SUM( COALESCE(x.qty_total,0) * COALESCE(b.composition,1) ) AS qty_total_rm,
            SUM( COALESCE(x.qty_output,0) * COALESCE(b.composition,1) ) AS qty_output_rm,
            SUM( COALESCE(x.qty_ng,0) * COALESCE(b.composition,1) ) AS qty_ng_rm,
        ...
        LEFT JOIN bom b ON x.item_fg_id = b.item_fg_id
        LEFT JOIN item_rm rm ON b.item_rm_id = rm.id
        LEFT JOIN item_familys fam ON rm.item_family_id = fam.id
        LEFT JOIN standard_price_rm spr ON rm.id = spr.item_rm_id
        WHERE rm.id LIKE '%$filter_items%'
        AND rm.division LIKE '%$filter_division%'
        AND rm.item_family_id LIKE '%$filter_item_family%'
        GROUP BY b.item_rm_id, rm.number, rm.name, rm.uom
        ORDER BY rm.number";
        ...
        $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#customers {border-collapse: collapse;width: 100%;font-size: 12px;}#customers td, #customers th {border: 1px solid #ddd;padding: 2px;}#customers tr:nth-child(even){background-color: #f2f2f2;}#customers tr:hover {background-color: #ddd;}#customers th {padding-top: 2px;padding-bottom: 2px;text-align: center;color: black;}</style><body>
            ...
            <br><br><br>
            <h3 style="margin:0;">REPORT NG RM</h3>
        ...
        <br>
            <table id="customers" border="1" style="font-size: 11px;">
                <tr>
                    ...
                    <th>% NG Process</th>
                    <th>Price</th>
                    <th>Amount <br> Total Production</th>
                    <th>Amount <br> Output Production</th>
                    <th>Amount <br> NG Process</th>
                </tr>';
        $no = 1;
        ...
        $totalNG = 0;
        $totalPrice = 0;
        $totalAmountTotalProduction = 0;
        $totalAmountOutputProduction = 0;
        $totalAmountNG = 0;
        $totalPersen = 0;
        foreach ($records as $record) {
            ...
            $total = round($record->qty_total_rm, 2);
            $output = round($record->qty_output_rm, 2);

            $amountTotal = $total * $record->price;
            $amountOutput = $output * $record->price;
            $ng_percent = $total > 0 ? number_format(($record->qty_ng_rm / $total) * 100, 2) . "%" : "0.00%";

            $totalTotalProduction += $total;
            $totalOutputProduction += $output;
            $totalNG += $record->qty_ng_rm;
            $totalPrice += $record->price;
            $totalAmountTotalProduction += $amountTotal;
            $totalAmountOutputProduction += $amountOutput;
            $totalAmountNG += $record->qty_ng_rm * $record->price;

            $html .= '  <tr>
                            <td style="text-align:center">' . $no . '</td>
                            <td style="mso-number-format:\@;">' . $record->number . '</td>
                            <td style="mso-number-format:\@;">' . $record->name . '</td>
                            <td colspan ="2"; style="text-align:right;">' . $record->item_family_name . '</td>
                            <td colspan ="2"; style="text-align:right;">' . number_format($total, 2) . '</td>
                            <td colspan ="2"; style="text-align:right;">' . number_format($output, 2) . '</td>
                            <td style="text-align:right;">' . number_format($record->qty_ng_rm, 2) . '</td>
                            <td style="text-align:right;">' . $ng_percent . '</td>
                            <td style="text-align:right;">' . number_format($record->price, 2) . '</td>
                            <td style="text-align:right;">' . number_format($amountTotal, 2) . '</td>
                            <td style="text-align:right;">' . number_format($amountOutput, 2) . '</td>
                            <td style="text-align:right;">' . number_format($record->qty_ng_rm * $record->price, 2) . '</td>
                        </tr>';

            if ($filter_display == "DETAIL") {
                $html .= '  <tr>
                                <td colspan="23" style="background:#D1FFC6; font-size: 11px;">
                                    <b>DETAIL OF ' . $record->number . ' - ' . $record->name . '</b>
                                </td>
                            </tr>';
                $html .= '  <tr>
                ...
                // ambil semua FG yang menggunakan RM ini
                $bom_fg = $this->crud->query("
                    SELECT b.item_fg_id, b.item_rm_id, b.composition,
                        fg.number AS fg_number, fg.name AS fg_name
                    FROM bom b
                    JOIN item_fg fg ON b.item_fg_id = fg.id
                    WHERE b.item_rm_id = '{$record->item_rm_id}'
                ");

                foreach ($bom_fg as $fg) {
                    $item_fg_id = $fg->item_fg_id;
                    $composition = $fg->composition;

                    // ambil produksi actual FG ini
                    $sql_ap = "
                        SELECT op.trans_date, op.wo_no, op.qty, op.qty_wip
                        FROM output_productions op
                        WHERE op.item_fg_id = '$item_fg_id'
                        AND op.trans_date BETWEEN '$filter_from' AND '$filter_to'
                    ";
                    $dataActualProductions = $this->crud->query($sql_ap);

                    // ambil NG FG ini
                    $sql_ng = "
                        SELECT ng.trans_date, ng.document, SUM(ng.qty_product) as qty_ng
                        FROM (
                            SELECT DISTINCT trans_date, document, item_fg_id, qty_product
                            FROM item_ng
                    ...
                    ";
                    $dataNgs = $this->crud->query($sql_ng);

                    // gabungkan data
                    $all_data = [];

                    foreach ($dataActualProductions as $ap) {
                        $all_data[] = [
                            'type' => 'ACTUAL PRODUCTION',
                            'date' => $ap->trans_date,
                            'wo_no' => $ap->wo_no,
                            'actual_production' => $ap->qty,
                            'qty_wip' => $ap->qty_wip,
                            'ng' => 0,
                        ];
                    }

                    foreach ($dataNgs as $ng) {
                        $all_data[] = [
                            'type' => 'PRODUCT NG',
                            'date' => $ng->trans_date,
                            'wo_no' => $ng->document,
                    ...
                    foreach ($all_data as $data) {
                        $qty_fg = $data['actual_production'] * $composition;
                        $qty_wip = $data['qty_wip'] * $composition;
                        $ng_rm = $data['ng'] * $composition;
                        $total_fg = $qty_fg + $qty_wip + $ng_rm;
                        ...
                        $html .= '  <tr>
                                        ...
                                        <td>' . $data['date']  . '</td>
                                        <td>' . $data['wo_no']  . '</td>
                                        <td style="text-align:right;">' . number_format($qty_fg, 4, ",", ".")  . '</td>
                                        <td style="text-align:right;">' . number_format($qty_wip, 4, ",", ".")  . '</td>
                                        <td style="text-align:right;">' . number_format($ng_rm, 4, ",", ".")  . '</td>
                                        <td style="text-align:right;">' . number_format($total_fg, 4, ",", ".")  . '</td>
                                    </tr>';
                        $nod++;
                    }
                }
        ...
        $html .= '<tr>
            ...
            <td style="text-align:right;">' . number_format($totalAmountTotalProduction, 2) . '</td>
            <td style="text-align:right;">' . number_format($totalAmountOutputProduction, 2) . '</td>
            <td style="text-align:right;">' . number_format($totalAmountNG, 2) . '</td>
        </tr>
        </tbody>';
    ...
    //         $format  = date("Ymd");
    //         header("Content-type: application/vnd-ms-excel");
    //         header("Content-Disposition: attachment; filename=report_ng_rm_summary_customer_$format.xls");
    //     }
    //     $filter_from = $this->input->get('filter_from');
    //     $filter_to   = $this->input->get('filter_to');
    //     $filter_items = $this->input->get('filter_items');
