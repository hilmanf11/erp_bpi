...
class Wip_receipts extends CI_Controller
{
    ...
    public function finalChecksheet()
    {
        ...
        FROM checksheets 
        WHERE `status` = '0' 
        ORDER BY `number` desc");// WHERE `status` = '0'
        echo json_encode($records);
    }
    ...
    public function print_label($checksheet_number)
    {
        ...
        //Cek Label
        $this->db->select('d.number, d.name, d.alias, a.qty, a.checksheet_label, f.trans_date, d.control_id, d.logo');// d.description,
        ...
        if ($wip_receipt_labels) {
            ...
            foreach ($wip_receipt_labels as $wip_receipt_label) {
                ...
                if ($wip_receipt_label->logo == "0") {
                    $img_bpi = '<img style="width:100%;" src="' . base_url("assets/image/BPI.png") . '" />';
                } else {
                    $img_bpi = '';
                }

                //Generate QRcode
                $qrcodes = $wip_receipt_label->alias . "|" . $wip_receipt_label->checksheet_label . "|" . $wip_receipt_label->qty . "|" . $wip_receipt_label->trans_date;
                ...
                $html .= '  <div style="width: 70mm; max-height:90mm; border:1px solid black; margin-bottom:5px;">
                                <table id="customers" border="1">
                                    ...
                                    <tr>
                                        <td style="width:15mm; height: 9mm;">' . $img_bpi . '</td>
    ...
    public function print_label_box($checksheet_number)
    {
        ...
        $checksheet_number = base64_decode($checksheet_number);
        //Cek Label
        $this->db->select('d.number, d.name, d.alias, a.qty, a.checksheet_label, f.trans_date, c.so_number, d.control_id, d.logo');//d.description,
        ...
        if ($wip_receipt_labels) {
            ...
            foreach ($wip_receipt_labels as $wip_receipt_label) {
                ...
                $qrcodes =$wip_receipt_label->checksheet_label . "|" . $wip_receipt_label->qty . "|" . $wip_receipt_label->trans_date;
                $this->createQrcode($qrcodes, "assets/image/qrcode/", $wip_receipt_label->checksheet_label);
                $html .= '  <div style="width: 70mm; max-height:90mm; border:1px solid black; margin-bottom:5px;">
                                <table id="customers" border="1">
                                    <tr>
