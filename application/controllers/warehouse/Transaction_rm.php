...
class Transaction_rm extends CI_Controller
{
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            if ($id === "0") {
                ...
                $this->db->join('(SELECT request_no, COUNT(status) as total_status_close FROM transaction_rm WHERE status = 1 GROUP BY request_no) g', 'a.request_no = g.request_no', 'left');
                $this->db->join('(SELECT request_no, COUNT(status) as total_status_open FROM transaction_rm WHERE status = 0 GROUP BY request_no) i', 'a.request_no = i.request_no', 'left');
    ...
    {
        $kanbans = $this->crud->reads('transaction_rm', [], ["request_no" => base64_decode($request_no)]);
        $kanban = $this->crud->read('transaction_rm', [], ["request_no" => base64_decode($request_no)]);
