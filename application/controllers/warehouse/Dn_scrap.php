...
class dn_scrap extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('warehouse/dn_scrap');
        } else {
            redirect('error_access');
        }
    }
  
    public function readItems($type)
    {
        ...
        if($type == 'Scrap'){
            $send = $this->crud->query("
                SELECT * 
                FROM item_rm 
                WHERE item_category_id = 'C06'
                    AND (number LIKE '%$post%' OR name LIKE '%$post%')
                ORDER BY number ASC
            ");
        }else{
            $send = $this->crud->query("
    ...
    {
        $data = $this->crud->query("SELECT DISTINCT document_no FROM dn_scrap WHERE `status` = '0' ORDER BY document_no ASC");
    ...
    public function create()
    {
        ...
        $autonumber = $autoID . "-" . $div . "-SC-" . $monthYear;
        ...
        foreach ($details as $row) {
            ...
            if ($this->crud->create('dn_scrap', $dataInsert)) {
                $inserted++;
            }
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('dn_scrap', $data);
        echo $send;
    }
    ...
    {
        $dn_scrap_total = $this->crud->reads('dn_scrap', [], ["document_no" => base64_decode($document_no)]);
        $dn_scraps = $this->crud->read('dn_scrap', [], ["document_no" => base64_decode($document_no)]);
        $customer_address = $this->crud->read('customer_address', [], ["customer_id" => $dn_scraps->customer_id, "plant" => $dn_scraps->plant]);

        $config = $this->db->get('config')->row();
        $config_iso = $this->db->get('config_iso')->row();
        $signatures = $this->db->get('signatures')->row();

        $approval = $this->crud->read('approvals', [], ["table_name" => "dn_scrap"]);
        ...
        //Config Page
        $rows = 8;
        $page = ceil(count($dn_scrap_total) / $rows);
        //Generate QRcode
        $this->createQrcode($dn_scraps->document_no, "assets/image/qrcode/");
        ...
        $html = '<html>
                    <head>
                        <title>' . $dn_scraps->document_no . '</title>
        ...
        $subtotal_karung = 0;
        $judul = "DELIVERY NOTE"; 
        // $form_iso = $config_iso->form_dn_scrap;
        for ($i = 0; $i < $page; $i++) {
            $this->db->select("a.*, 
            ...
            if ($dn_scraps->updated_date != null) {
                $revision_date = $dn_scraps->updated_date;
            } else {
                $revision_date = $dn_scraps->created_date;
            }
                        ...
                        <div style="border: 0px solid black; width:100%;">
                            <div style="padding:10px;">
                                <center>
                                    ...
                                    <small>NO : ' . @$dn_scraps->document_no . '</small>
                                </center>
                                ...
                                <table style="width:100%; font-size:12px; margin-bottom:10px;">
                                    <tr>
                                        ...
                                        <td style="text-align:right; padding-right: 20px;" rowspan="7">
                                            ...
                                            Transaction Date:<br><b>' . date("d F Y", strtotime($dn_scraps->transaction_date)) . '</b><br>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td width="50">Doc No</td>
                                        <td width="10">:</td>
                                        <td><b>' . @$dn_scraps->document_no . '</b></td>
                                    </tr>
                                </table>
                                <table id="customers">
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2" width="80" style="text-align:center;">Qty Karung</th>
            ...
            foreach ($records as $record) {
                $html .= '  
                            <tr>    
                                ...
                                <td style="text-align:right;">' . $record['qty_karung'] . '</td>
                                <td style="text-align:center;">' . $record['remarks'] . '</td>
                            </tr>';
                $row++;
                $no++;
                $subtotal += $record['qty'];
                $subtotal_karung += $record['qty_karung'];
            }
            $html .= '<tr>
                <td colspan="6" style="text-align:center; font-weight:bold;">TOTAL</td>
                <td style="text-align:right; font-weight:bold;">' . number_format($subtotal, 2, ",", ".") . '</td>
                <td style="text-align:right; font-weight:bold;">' . number_format($subtotal_karung, 2, ",", ".") . '</td>
                <td>-</td>
            </tr>';
            if (($i + 1) == $page) {
     ...
     public function uploadatt()
    {
        ...
        $uploadDir = 'assets/image/dn_scrap/';
