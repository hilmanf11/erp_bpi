...
class Receipt_crusher extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('warehouse/receipt_crusher');
        } else {
    ...
    {
        $records = $this->crud->query("SELECT `period` FROM receipt_crusher WHERE `status` = '0' GROUP BY `period`");
        echo json_encode($records);
    }

    public function readWp($period)
    {
        $records = $this->crud->query("SELECT workorder FROM receipt_crusher WHERE `status` = '0' and `period` = '$period' GROUP BY workorder");
        echo json_encode($records);
    }
    ...
    public function readRequestNo($period, $workorder)
    {
        ...
        $records = $this->crud->query("SELECT request_no FROM receipt_crusher WHERE status = '0' and `period` = '$period' and workorder = '$workorder' GROUP BY `request_no`");
        echo json_encode($records);
    }

    public function readRequestNos()
    {
        $records = $this->crud->query("SELECT request_no FROM receipt_crusher WHERE status = '0' GROUP BY `request_no`");
        echo json_encode($records);
    }
    ...
    public function readItemRm()
    {
        ...
        FROM item_rm a
        LEFT JOIN master_mpq_material_crushers b ON a.id = b.item_rm_id
        WHERE a.status = '0' 
            AND a.division = 'INJ' 
    ...
    {
        $records = $this->crud->query("SELECT b.number, b.name, b.id FROM receipt_crusher a JOIN item_rm b ON a.item_rm_id = b.id WHERE b.item_family_id = '$product_family_id' GROUP BY b.id ORDER BY b.number asc");
        echo json_encode($records);
    }
    ...
    public function request_no($trans_date)
    {
        ...
        $sqlGetID   = $this->db->query("SELECT max(request_no) as kode FROM receipt_crusher WHERE request_no like '%$datenow%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
    ...
    public function datatableUpdate($request_no){
        ...
        $this->db->from('receipt_crusher a');
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $filter_request_date = $this->input->get('filter_request_date');
            ...
            if ($id === "0") {
                ...
                $this->db->join('(SELECT request_no, COUNT(status) as total_status_close FROM receipt_crusher WHERE status = 1 GROUP BY request_no) g', 'a.request_no = g.request_no', 'left');
                $this->db->join('(SELECT request_no, COUNT(status) as total_status_open FROM receipt_crusher WHERE status = 0 GROUP BY request_no) i', 'a.request_no = i.request_no', 'left');
                ...
                if($filter_request_date != ""){
                    $this->db->where('a.request_date', $filter_request_date);
                }
                if($filter_status != ""){
                ...
                $this->db->from('receipt_crusher a');
                $this->db->join('item_rm b', 'a.item_rm_id = b.id');
                $this->db->join('scan_item_receipt_crusher c', 'a.request_id = c.request_id and a.item_rm_id = c.item_rm_id','left');
                $this->db->where('a.deleted', 0);
    ...
    {
        $sqlGetID   = $this->db->query("SELECT max(request_id) as kode FROM receipt_crusher WHERE request_id like '%$receipt_no%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($post['qty'] == 0) {
                    ...
                    $datas = $this->crud->reads('receipt_crusher', [], ["request_no" => $request_no, "item_rm_id" => $item_rm_id]);

                    if(count($datas) > 0){
                        $send = $this->crud->update('receipt_crusher', ["request_no" => $request_no, "item_rm_id" => $item_rm_id], $post);
                        echo $send;
                    }else{
                        $send   = $this->crud->create('receipt_crusher', array_merge($post, ["request_id" => $this->request_id($post['request_no'])]));
                        echo $send;
                    }
                }
            } else {
    ...
    public function print_label($request_id)
    {
        ...
        $po_receipt = $this->crud->read('receipt_crusher', [], ["request_id" => $request_id]);
        $qty_receipt = $po_receipt->qty;
        $item_rm_id = $po_receipt->item_rm_id;
        $lot_no = $po_receipt->lot_no;
        $shift = $po_receipt->shift;
        $request_name = $po_receipt->request_name;
        $request_date = $po_receipt->request_date;
        ...
        //Cek Label
        $po_receipt_label = $this->crud->reads('receipt_crusher_labels', [], ["request_id" => $request_id]);
        if (!$po_receipt_label) {
            for ($i = 0; $i < $po_receipt->label; $i++) {
                //Read Label ID
                $sqlGetID = $this->db->query("SELECT max(label_no) as kode FROM receipt_crusher_labels WHERE request_id = '$request_id'");
                $rowID = $sqlGetID->row();
                $label = $rowID->kode;
                if ($label == NULL) {
                ...
                $send = $this->crud->create('receipt_crusher_labels', $arrLabel);
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('receipt_crusher', ["request_id" => $data['request_id']]);
        $delete = $this->crud->delete('receipt_crusher_labels', ["request_id" => $data['request_id'], "item_rm_id" => $data['item_rm_id']]);
        echo $send;
    }

     public function print_doc($request_no)
    {
        $kanbans = $this->crud->reads('receipt_crusher', [], ["request_no" => base64_decode($request_no)]);
        $kanban = $this->crud->read('receipt_crusher', [], ["request_no" => base64_decode($request_no)]);
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=receipt_crusher_$format.xls");
        }
        // $filter_period = $this->input->get('filter_period');
        // $filter_workorder   = $this->input->get('filter_workorder');
        $filter_request_no = $this->input->get('filter_request_no');
