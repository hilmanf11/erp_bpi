...
class Item_receipts_fg extends CI_Controller
{
    ...
    public function getDocumentNo()
    {
        if ($this->input->get()) {
            ...
            $this->db->select('COALESCE(f.checksheet_number, h.checksheet_number) as checksheet_number, b.wo_no, d.number as item_number, d.name as item_name, 
    ...
    public function getChecksheetLabel()
    {
        if ($this->input->post()) {
            ...
            if(!$records){
                ...
                if(!$records){
                    $this->db->select('COALESCE(a.qty, 0) as qty, a.label_no as checksheet_label, a.item_fg_id, a.packing_date , "NBFG" as type');
