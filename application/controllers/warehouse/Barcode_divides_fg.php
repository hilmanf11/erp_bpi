...
class Barcode_divides_fg extends CI_Controller
{
    ...
    public function getSerial()
    {
        if ($this->input->post()) {
            ...
            $this->db->select('a.*, b.trans_date, c.number, c.name');
    ...
    public function print()
    {
        ...
        if (!$records) {
            ...
            (CASE 
                ...
                ELSE b.lot_no 
            END) as lot_no, 
            b.qc_1, b.qc_2, b.op_1, b.op_2, b.qcnumber_1, b.qcnumber_2, b.opnumber_1, b.opnumber_2, h.location,"LABEL BOXS" as label_desc, b.packing'); // d.description,    

            $this->db->from('barcode_divides_fg a');
            $this->db->join('wip_receipt_boxs e', 'a.reff = e.checksheet_label');
            $this->db->join('checksheets b', 'e.checksheet_number = b.number');
            $this->db->join('production_schedules c', 'b.wo_no = c.wo_no','left');
            $this->db->join('item_fg d', 'b.item_fg_id = d.id');
            $this->db->join('warehouse_location_items g', 'd.id = g.item_fg_id', 'left');
            ...
            if (!$records) {
                $this->db->select('b.*,d.number,d.number_customer as item_number_customer, d.name, e.location, e.area, d.color, d.uom,b.cut_off_date, d.number as item_number, d.name as item_name, d.alias, d.logo, d.uom, 
                e.location, b.packing, a.qty, a.label_divided, b.packing_date as trans_date');
                $this->db->from('barcode_divides_fg a');
                $this->db->join('new_barcode_fg b', 'a.label_no = b.label_no');
                $this->db->join('item_fg d', 'b.item_fg_id = d.id');
