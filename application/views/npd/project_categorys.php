...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'name',width:200,halign:'center'">Category Name</th>
            <th rowspan="2" data-options="field:'code',width:200,halign:'center'">Category Code</th>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('npd/project_categorys/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('npd/project_categorys/create') ?>';
        $('#frm_insert').form('clear');
        
        $.ajax({
            type : "post",
            url : "<?= base_url('npd/project_categorys/autoid')?>",
            dataType : "html",
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('npd/project_categorys/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('npd/project_categorys/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('npd/project_categorys/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/project_categorys/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
