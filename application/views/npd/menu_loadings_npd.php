<div id="dlg_help" class="easyui-dialog" title="About Menu" data-options="closed: true,modal:true" style="width: 800px; height: 500px; left: 10px; top: 20px;">
    <div class="easyui-accordion" style="width:100%; height: 100%;">
        <div title="RELATIONS" style="padding: 20px;">
            <ul>
                <li>The Data Product No is taken from <b>NPD > Engineering > Master Finish Good</b></li>
...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    ...
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'cavity_standard',width:120,halign:'center'">Cavity Standard</th>
...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 400px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate enctype="multipart/form-data">
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Cavity Standard</span>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('npd/menu_loadings_npd/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('npd/menu_loadings_npd/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('npd/menu_loadings_npd/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('npd/menu_loadings_npd/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_menu_loadings_npd.xls') ?>');
    }
    //PRINT PDF
    function pdf() {
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('npd/menu_loadings_npd/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/menu_loadings_npd/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
    ...
    $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('npd/menu_loadings_npd/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('npd/menu_loadings_npd/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('npd/menu_loadings_npd/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('npd/menu_loadings_npd/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('npd/menu_loadings_npd/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
