...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('npd/bom_npd/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('npd/bom_npd/create') ?>';
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('npd/bom_npd/readItem'); ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
                            ...
                            onSelect: function (value, rows) {
                                ...
                                $.when(
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('npd/bom_npd/readWeight'); ?>",
                                    ...
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('npd/bom_npd/readRunner'); ?>",
                                        data: "item_fg_id=" + item_fg_id,
                                        dataType: "json",
    ...
    function removeit() {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url('npd/bom_npd/delete') ?>',
            data: {
    ...
    function update() {
        ...
        if (row) {
            ...
            addTable('<?= base_url('npd/bom_npd/datatableUpdates?item_fg_id=') ?>' + window.btoa(row.item_fg_id));
        } else {
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/bom_npd/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('npd/bom_npd/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('npd/bom_npd/print/excel') ?>' + url);
    }
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/bom_npd/datatables') ?>',
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('npd/bom_npd/datatableDetails?number=') ?>' + window.btoa(row.item_fg_number) + "&filter_item_rm_id=" + window.btoa(filter_item_rm_id),
                    singleSelect: true,
                    rownumbers: true,
                    columns: [
                        [{
                ...
                handler: function() {
                    ...
                    for (let i = 0; i < totalrows; i++) {
                        if (rows[i].item_rm_id) {
                            ...
                            if(rows[i].type_item == "SA"){
                                var url_save = "<?= base_url('npd/bom_npd/create_SA') ?>";
                            }else{
                                var url_save = "<?= base_url('npd/bom_npd/create') ?>";
                            }
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('npd/bom_npd/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('npd/bom_npd/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('npd/bom_npd/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('npd/bom_npd/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('npd/bom_npd/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
