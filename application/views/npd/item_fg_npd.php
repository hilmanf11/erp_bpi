...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('npd/item_fg_npd/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('npd/item_fg_npd/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('npd/item_fg_npd/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('npd/item_fg_npd/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_item_fg_npd.xls') ?>');
    }
    //PRINT PDF
    function pdf() {
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('npd/item_fg_npd/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/item_fg_npd/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
    ...
    $('#division_id').combobox({
        ...
        onSelect: function(division) {
            $.ajax({
                type: "post",
                url: "<?= base_url('npd/item_fg_npd/autoid/') ?>" + "/" + division.number,
                dataType: "html",
    ...
    $('#item_family_id').combobox({
        url:'<?= base_url('npd/item_fg_npd/readFamily'); ?>',
        valueField:'id',
        textField:'name',
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('npd/item_fg_npd/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('npd/item_fg_npd/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('npd/item_fg_npd/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('npd/item_fg_npd/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('npd/item_fg_npd/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
