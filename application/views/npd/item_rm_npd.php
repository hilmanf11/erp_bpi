...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('npd/item_rm_npd/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('npd/item_rm_npd/create') ?>';
    ...
    function update() {
        ...
        $('#item_category_id').combobox({
            ...
            onSelect: function (category) {
                ...
                $('#item_family_id').combogrid({
                    url:'<?= base_url('npd/item_rm_npd/readFamily/'); ?>' + category.id,
                    panelWidth: 300,
                    idField: 'id',
                    textField: 'name',
                    mode: 'remote',
        ...
        if (row) {
            ...
            url_save = '<?= base_url('npd/item_rm_npd/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('npd/item_rm_npd/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('npd/item_rm_npd/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('npd/item_rm_npd/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
        ...
        onSelect: function(category){
            ...
            $('#item_family_id').combogrid({
                ...
                onSelect: function(value, rows) {
                    ...
                    $.ajax({
                        type : "post",
                        url : "<?= base_url('npd/item_rm_npd/autoid/')?>" + category.number + "/" + rows.number,
                        dataType : "html",
            ...
            //         $.ajax({
            //             type : "post",
            //             url : "<?= base_url('npd/item_rm_npd/autoid/')?>" + category.number + "/" + family.number,
            //             dataType : "html",
            //             success : function(response){
            //                 $('#id').textbox('setValue', response);
            //             }
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('npd/item_rm_npd/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                ...
                $.ajax({
                    url: '<?= base_url('npd/item_rm_npd/uploadclearFailed') ?>',
                    type: 'POST',
                ...
                $.ajax({
                    url: '<?= base_url('npd/item_rm_npd/upload') ?>',
                    type: 'POST',
                    data: formData,
                    dataType: 'json',
                ...
                function requestData(total, data_array, number = 1, success = 0, failed = 0) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('npd/item_rm_npd/uploadCreate') ?>",
                        ...
                        success: function(result_item_create) {
                            ...
                            if (result_item_create.theme === "success") {
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: "<?= base_url('npd/item_rm_npd/uploadcreateFailed') ?>",
