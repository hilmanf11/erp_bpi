<div id="dlg_help" class="easyui-dialog" title="About Menu" data-options="closed: true,modal:true" style="width: 800px; height: 500px; left: 10px; top: 20px;">
    <div class="easyui-accordion" style="width:100%; height: 100%;">
        ...
        <div title="CONDITIONS" style="padding: 20px;">
            <ul>
                <li><b>Composition :</b> if Part Type is <b>INDIRECT</b> then do not fill the Composition</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('master/consumable_part/create') ?>';
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('master/consumable_part/readItem'); ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
                            ...
                            onSelect: function (value, rows) {
                                ...
                                $.when(
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('master/consumable_part/readWeight'); ?>",
                                    ...
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('master/consumable_part/readRunner'); ?>",
                                        data: "item_fg_id=" + item_fg_id,
                                        dataType: "json",
    ...
    function update() {
        ...
        if (row) {
            ...
            addTable('<?= base_url('master/consumable_part/datatableUpdates?item_fg_id=') ?>' + window.btoa(row.item_fg_id));
        } else {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_consumable_part.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/consumable_part/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('master/consumable_part/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('master/consumable_part/print/excel') ?>' + url);
    }
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/consumable_part/datatables') ?>',
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('master/consumable_part/datatableDetails?number=') ?>' + window.btoa(row.item_fg_number) + "&filter_item_rm_id=" + window.btoa(filter_item_rm_id),
                    singleSelect: true,
                    rownumbers: true,
                    columns: [
                        [{
                ...
                handler: function() {
                    ...
                    for (let i = 0; i < totalrows; i++) {
                        if (rows[i].item_rm_id) {
                            ...
                            if(rows[i].type_item == "SA"){
                                var url_save = "<?= base_url('master/consumable_part/create_SA') ?>";
                            }else{
                                var url_save = "<?= base_url('master/consumable_part/create') ?>";
                            }
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('master/consumable_part/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('master/consumable_part/upload') ?>',
                    onSubmit: function() {
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('master/consumable_part/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('master/consumable_part/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
