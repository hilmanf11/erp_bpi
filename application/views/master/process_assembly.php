...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('master/process_assembly/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('master/process_assembly/create') ?>';
        $('#frm_insert').form('clear');

        $.ajax({
            type : "post",
            url : "<?= base_url('master/process_assembly/autoid')?>",
            dataType : "html",
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('master/process_assembly/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('master/process_assembly/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('master/process_assembly/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/process_assembly/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
