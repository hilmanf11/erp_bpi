    ...
    <div style="width: 100%;">
        <fieldset style="width: 60%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
            ...
            <div style="width: 50%; float: left;">
                ...
                <div class="fitem">
                    <span style="width:35%; display:inline-block;">Transaction Date</span>
                    <input style="width:29%;" id="filter_from" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser, editable:false"> To
                    <input style="width:27%;" id="filter_to" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser, editable:false">
                </div>
                <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('control/wip_adjustment_fg/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //Add Data
    function add() {
    ...
    function request_no(reqDate = "") {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('control/wip_adjustment_fg/request_no') ?>/" + window.btoa(request_date),
            dataType: "html",
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('control/wip_adjustment_fg/readItemFg') ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
        ...
        if (row) {
            if(row.state == "closed"){
                ...
                $("#transaction_id").combobox({
                    url: '<?= base_url('master/wip_transactions/reads/') ?>',
                ...
                addTable('<?= base_url('control/wip_adjustment_fg/datatableUpdate/') ?>' + window.btoa(row.request_no));
            }else{
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('control/wip_adjustment_fg/delete') ?>',
                            data: {
                                id: row.id,
    ...
    function filter() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('control/wip_adjustment_fg/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('control/wip_adjustment_fg/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        url = "?&filter_request_no=" + filter_request_no + "&filter_product_no=" + btoa(filter_product_no) + "&filter_from=" + filter_from + "&filter_to=" + filter_to + "&filter_transaction_type=" + filter_transaction_type;
        window.location.assign('<?= base_url('control/wip_adjustment_fg/print/excel') ?>' + url);
    }

    function print_kanban() {
        var row = $('#dg').treegrid('getSelected');
        if (row) {
            window.open("<?= base_url('control/wip_adjustment_fg/print_kanban/') ?>" + window.btoa(row.request_no));// + "/" + window.btoa(operation), "_blank"
        }else{
            toastr.warning("Please select one of the data in the table first!", "Information");
        }
    }
    ...
    $(function() {
        addTable();
        $('#dg').treegrid({
            url: '<?= base_url('control/wip_adjustment_fg/datatables') ?>',
            pagination: true,
            rownumbers: true,
            idField: 'id',
                    ...
                    if (totalrows <= 0) {
                        ...
                        for (let i = 0; i < totalrows; i++) {
                            if (rows[i].item_fg_id) {
                                $.ajax({
                                    type: "post",
                                    url: '<?= base_url('control/wip_adjustment_fg/create') ?>',
                                    data: {
        ...
        $("#filter_period").combobox({
            url: '<?= base_url('control/wip_adjustment_fg/readPeriod') ?>',
            valueField: 'period',
            textField: 'period',
            prompt: "Choose period",
            icons: [{
            ...
            onSelect: function(period) {
                $("#filter_workorder").combobox({
                    url: '<?= base_url('control/wip_adjustment_fg/readWp/') ?>' + period.period,
                    ...
                    onSelect: function(wp) {
                        $("#filter_request_no").combobox({
                            url: '<?= base_url('control/wip_adjustment_fg/readRequestNo/') ?>' + period.period + '/' + window.btoa(wp.workorder),
        ...
        $("#filter_request_no").combobox({
            url: '<?= base_url('control/wip_adjustment_fg/readRequestNos/') ?>',
            valueField: 'request_no',
            textField: 'request_no',
            prompt: "Choose Request No",
            icons: [{
        ...
        $('#filter_product_no').combogrid({
            url: '<?= base_url('control/wip_adjustment_fg/readProducts/') ?>',
            panelWidth: 420,
            idField: 'id',
            textField: 'number',
            mode: 'remote',
    ...
    function BtnPrintLabel(val, row) {
        return '<a style="text-decoration: none; font-weight:bold;" target="_blank" href="<?= base_url('control/wip_adjustment_fg/print_label/') ?>' + window.btoa(row.id) + '"><i class="fa fa-print"></i> Print</a>';
    }
