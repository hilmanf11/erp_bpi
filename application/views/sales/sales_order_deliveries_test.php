...
<!-- DIALOG DELIVERY -->
<div id="dlg_delivery" class="easyui-dialog" title="Working Calendar" data-options="fit:true, modal:true, closed:true" style="width:100%; background: #F4F4F4; padding: 10px;">
    <form id="frm_insert" method="post">
        <fieldset style="width: 30%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <a href="javascript:;" class="easyui-linkbutton" onclick="filter_data()"><i class="fa fa-search"></i> Filter Data</a>
                <a href="javascript:;" class="easyui-linkbutton" onclick="add()"><i class="fa fa-save"></i> Save Data</a>
            </div>
        </fieldset>
        <fieldset style="width: 60%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px; float: left;">
            ...
            <div style="width: 30%; float: left;">
                ...
                <div class="fitem">
                    ...
                    <input style="width:60%;" name="uom" id="uom" readonly class="easyui-textbox">
                </div>
            </div>
            <div style="width: 30%; float: left;">
                <div class="fitem">
                        <span style="width:35%; display:inline-block;">Qty SO</span>
                        <input style="width:60%;" name="qty_so" id="qty_so" readonly class="easyui-textbox">
                </div>
            </div>
        </fieldset>
        <div id="calendar">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('sales/sales_order_deliveries_test/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        $('#frm_insert').form('submit', {
            url: '<?= base_url('sales/sales_order_deliveries_test/create') ?>',
            method: 'POST',
            onSubmit: function() {
            ...
            success: function(result) {
                ...
                if (result.theme == "success") {
                    ...
                    calendars(filter_month, filter_year, customer_id, sales_order_no, item_fg_id);
    ...
    function calendars(month = "", year = "", customer_id = "", sales_order_no = "", item_fg_id = "") 
    {
        $.ajax({
            type: "post",
            url: "<?= base_url('sales/sales_order_deliveries_test/calendars') ?>",
            data: {
                month: month,
                year: year,
    ...
    function hkw(month = "", year = "") {
        $.ajax({
            type: "post",
            url: "<?= base_url('sales/sales_order_deliveries_test/hkw') ?>",
    ...
    function btnDelivery(val, row) {
        var delivery = "delivery('" + row.customer_id + "','" + row.sales_order_no + "','" + row.item_fg_id + "','" + row.item_fg_number + "','" + row.qty + "','" + row.item_fg_name + "','" + row.uom + "')";
        return '<a class="btn btn-primary w-100" onClick="' + delivery + '" style="pointer-events: visible; opacity:1;"><i class="fa fa-truck"></i></a>';
    }

    function delivery(customer_id, sales_order_no, item_fg_id, item_fg_number, qty, item_fg_name, uom) {
        ...
        $("#item_fg_name").textbox('setValue', item_fg_name);
        $("#qty_so").textbox('setValue', qty);
        $("#uom").textbox('setValue', uom);

        var customer_id = $("#customer_id").textbox('getValue');
        var sales_order_no = $("#sales_order_no").textbox('getValue');
        var item_fg_id = $("#item_fg_id").textbox('getValue');
        var qty_item = $("#qty_so").textbox('getValue');

        console.log(qty_item);

        calendars($("#filter_month").combobox('getValue'), $("#filter_year").numberbox('getValue'), customer_id, sales_order_no, item_fg_id);
    }
    ...
    //             url_save = '<?= base_url('sales/sales_order_deliveries_test/update') ?>?id=' + btoa(row.id);
    //         } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('sales/sales_order_deliveries_test/delete') ?>',
                            data: {
                                id: row.id
                            },
                            success: function(result) {
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('sales/sales_order_deliveries_test/datatables') ?>' + url,
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('sales/sales_order_deliveries_test/datatableDetails?sales_order_no=') ?>' + window.btoa(row.sales_order_no),
                    singleSelect: true,
                    rownumbers: true,
                    columns: [
                        [{
        ...
        $("#printout").attr('src', '<?= base_url('sales/sales_order_deliveries_test/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('sales/sales_order_deliveries_test/print/excel') ?>' + url);
    }
    ...
    $('#filter_customer_id').combobox({
        ...
        onSelect: function(customer) {
            $('#filter_sales_order_no').combobox({
                url: '<?= base_url('sales/sales_order_deliveries_test/readSalesOrder/'); ?>' + customer.id,
            ...
            $('#filter_customer_order_no').combobox({
                url: '<?= base_url('sales/sales_order_deliveries_test/readCustomerOrder/'); ?>' + customer.id,
            ...
            $('#filter_item_fg').combogrid({
                url: '<?= base_url('sales/sales_order_deliveries_test/readProductNo/'); ?>' + customer.id,
                panelWidth: 400,
                idField: 'id',
                textField: 'number',
                mode: 'remote',
    ...
    $('#filter_customer_order_no').combobox({
        url: '<?= base_url('sales/sales_order_deliveries_test/readCustomerOrders'); ?>',
        valueField: 'customer_order_no',
        textField: 'customer_order_no',
        prompt: 'Choose All',
        icons: [{
    ...
    $('#filter_sales_order_no').combobox({
        url: '<?= base_url('sales/sales_order_deliveries_test/readSalesOrders'); ?>',
        valueField: 'sales_order_no',
        textField: 'sales_order_no',
        prompt: 'Choose All',
        icons: [{
    ...
    $('#filter_item_fg').combogrid({
        url: '<?= base_url('sales/sales_order_deliveries_test/readProductNos'); ?>',
        panelWidth: 400,
        idField: 'id',
        textField: 'number',
        mode: 'remote',
