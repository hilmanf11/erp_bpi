...
<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'btn',width:80,halign:'center',align:'right',formatter:btnHistories">History</th>
            ...
            <th rowspan="2" data-options="field:'cutoff',width:80,halign:'center'">Cut Off</th>
            <th rowspan="2" data-options="field:'uom',width:80,align:'center'">Uom</th>
            <th rowspan="2" data-options="field:'transfer_from',width:100,align:'center'">From</th>
            <th rowspan="2" data-options="field:'transfer_to',width:100,align:'center'">To</th>
            <th rowspan="2" data-options="field:'qty_from',width:80,halign:'center',align:'right',formatter:numberFormat">Qty From</th>
            <th rowspan="2" data-options="field:'qty_to',width:80,halign:'center',align:'right',formatter:numberFormat">Qty To</th>
...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 500px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Cut Off</span>
                <input style="width:60%;" name="cutoff"  id="cutoff" value="<?= date("Y-m-d") ?>" required class="easyui-datebox" data-options="formatter:myformatter,parser:myparser, editable:false">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Part No</span>
           ...
           <div class="fitem">
                ...
                <input style="width:60%;" name="transfer_from" id="transfer_from" required class="easyui-combobox">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Transfer To</span>
                <input style="width:60%;" name="transfer_to" id="transfer_to" required class="easyui-combobox">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Qty From</span>
                <input style="width:30%;" name="qty_from" id="qty_from" data-options="precision:2" required class="easyui-numberbox">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Qty To</span>
                <input style="width:30%;" name="qty_to" id="qty_to" data-options="precision:2" required class="easyui-numberbox">
            </div>
        </fieldset>
    </form>
</div>
...
<!-- Detail Histories -->
<div id="dlg_history" class="easyui-dialog" title="Transfer Histories" data-options="closed: true,modal:true" style="width: 1000px; height: 450px; top: 20px;">
    <table id="dg_history" class="easyui-datagrid" style="width:100%;">
        <thead>
            <tr>
                <th data-options="field:'item_rm_number',width:150,halign:'center'">Part No</th>
                <th data-options="field:'item_rm_name',width:100,halign:'center'">Part Name</th>
                <th data-options="field:'trans_date',width:80,halign:'center'">Trans Date</th>
                <th data-options="field:'cutoff',width:80,halign:'center'">Cut Off</th>
                <th data-options="field:'transfer_from',width:100,halign:'center'">From</th>
                <th data-options="field:'transfer_to',width:100,halign:'center'">To</th>
                <th data-options="field:'qty_from',width:80,halign:'center',align:'right',formatter:numberFormat">Qty From</th>
                <th data-options="field:'qty_to',width:80,halign:'center',align:'right',formatter:numberFormat">Qty To</th>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('warehouse/upload_stock_whs_tf/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('warehouse/upload_stock_whs_tf/create') ?>';
        $('#frm_insert').form('clear');
        // $("#item_rm_id").combogrid('enable');
        $("#cutoff").datebox('setValue', '<?= date("Y-m-d") ?>');
    ...
    function update() {
        ...
        if (row) {
            ...
            // url_save = '<?= base_url('warehouse/upload_stock_whs_tf/update') ?>?id=' + btoa(row.id);
            url_save = '<?= base_url('warehouse/upload_stock_whs_tf/create') ?>';
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('warehouse/upload_stock_whs_tf/delete') ?>',
                            data: {
                                id: row.id,
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_upload_stock_whs_tf.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('warehouse/upload_stock_whs_tf/datatables') ?>' + url,
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
        ...
        $("#printout").attr('src', '<?= base_url('warehouse/upload_stock_whs_tf/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('warehouse/upload_stock_whs_tf/print/excel') ?>' + url);
    }
    ...
    $('#transfer_from').combobox({
        url: '<?= base_url('warehouse/upload_stock_whs_tf/readArea/'); ?>',
        valueField: 'area',
        textField: 'area',
    ...
    function btnHistories(val, row) {
        var history = "viewHistories('" + row.item_rm_id + "')";
        return '<a class="btn btn-primary w-100" onClick="' + history + '" style="pointer-events: visible; opacity:1;"><i class="fa fa-eye"></i> View</a>';
    }

    function viewHistories(item_rm_id) {
        $("#dlg_history").dialog('open');
        $('#dg_history').datagrid({
            url: '<?= base_url('warehouse/upload_stock_whs_tf/datatableHistories?item_rm_id=') ?>' + btoa(item_rm_id),
            pagination: false,
            rownumbers: true,
        });
    }
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('warehouse/upload_stock_whs_tf/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('warehouse/upload_stock_whs_tf/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('warehouse/upload_stock_whs_tf/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('warehouse/upload_stock_whs_tf/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('warehouse/upload_stock_whs_tf/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
