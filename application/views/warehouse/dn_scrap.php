    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div style="width: 50%; float: left;">
                ...
                <!-- <div class="fitem">
                    <span style="width:30%; display:inline-block;">Recycle</span>
                    <input style="width:60%;" name="recycle" id="recycle" class="easyui-combobox">
                </div>
                <div class="fitem">
                    <span style="width:30%; display:inline-block;">Lot No</span>
                    <input style="width:60%;" name="lot_no" id="lot_no" class="easyui-textbox">
                </div> -->
                <div class="fitem">
                    <span style="width:30%; display:inline-block;">Type</span>
                    <input style="width:60%;" name="type" id="type" class="easyui-combobox">
                </div>
            </div>
        </fieldset>
        <table id="dg2" class="easyui-datagrid" style="width:100%; height: 350px;" title="Item Lists" toolbar="#toolbar2"></table>
    </form>
</div>
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('warehouse/dn_scrap/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('warehouse/dn_scrap/create') ?>';
        $('#frm_insert').form('clear');
        $('#transaction_date').datebox('setValue', '<?= date("Y-m-d") ?>');
    }

    function addTable(type, link = "") {
        var dg =  $('#dg2').datagrid({
            url: link,
            singleSelect: true,
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('warehouse/dn_scrap/readItems/'); ?>' + type,
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
                            ...
                            onSelect: function(value, row) {
                                ...
                                var isDuplicate = allRows.some(function(r) {
                                    return r.item_rm_number === row.number;
                                });
                    ...
                    editor:{
                        ...
                        options:{
                            ...
                            onChange: function(){
                                ...
                                $.ajax({
                                    url: '<?= base_url('warehouse/dn_scrap/uploadatt') ?>',
                                    type:'post',
                                    data: formData,
                                    contentType:false,
                                    processData:false,
    ...
    function removeit() {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url('warehouse/dn_scrap/delete') ?>',
            data: {
    ...
    function update() {
        ...
        if (row) {
            ...
            $('#plant').combogrid({
                url: '<?= base_url('warehouse/dn_scrap/readPlant/'); ?>' + window.btoa(row.customer_id) ,
                panelWidth: 420,
                idField: 'id',
                textField: 'plant',
                mode: 'remote',
            ...
            // Load tabel terkait
            addTable('<?= base_url('warehouse/dn_scrap/datatableUpdates?customer_id=') ?>' + window.btoa(row.customer_id) + "&division=" + window.btoa(row.division) + "&plant=" + window.btoa(row.plant));
        } else {
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('warehouse/dn_scrap/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('warehouse/dn_scrap/print') ?>' + url);
    }
    ...
    function excel() {
       var filter_from = $("#filter_from").datebox('getValue');
        ...
        window.location.assign('<?= base_url('warehouse/dn_scrap/print/excel') ?>' + url);
    }
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('warehouse/dn_scrap/datatables') ?>',
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('warehouse/dn_scrap/datatableDetails?number=') ?>' + window.btoa(row.document_no) + "&filter_item_rm_id=" + window.btoa(filter_item_rm_id),
                    singleSelect: true,
                    rownumbers: true,
                    fitColumns: true,
                    columns: [
        ...
        $('#dlg_insert').dialog({
            buttons: [{
                ...
                handler: function() {
                    ...
                    // var recycle          = $("#recycle").combobox('getValue');
                    ...
                    var detailsToSave = rows.map(function(r) {
                        return {
                            item_rm_id: r.item_rm_id,
                            qty: r.qty,
                            qty_karung: r.qty_karung,
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url('warehouse/dn_scrap/create') ?>",
                        data: {
                            header: headerData,
    ...
    $('#customer_id').combogrid({
        ...
        onSelect: function(value, rows) {
            $('#plant').combogrid({
                url: '<?= base_url('warehouse/dn_scrap/readPlant/'); ?>' + window.btoa(rows.id),
                panelWidth: 420,
                idField: 'plant',
                textField: 'plant',
                mode: 'remote',
    ...
    // function setLotNo(recycleValue) {
    //     var recycle = recycleValue || $('#recycle').combobox('getValue'); // gunakan nilai dari parameter jika ada
    //     var tgl = $('#transaction_date').datebox('getValue');
    ...
    //         $('#lot_no').textbox('setValue', formatDate + '-' + recycle);
    //     }
    // }
    ...
    //     textField: 'text',
    //     panelHeight: 'auto',
    //     prompt: 'Choose Recycle',
    //     onSelect: function(record) {
    //         setLotNo(record.value); 
    //     }
    // });

    // $('#transaction_date').datebox({
    //     onSelect: function() {
    //         setLotNo();
    //     }
    // });
    ...
    $('#filter_document_no').combobox({
        url: '<?= base_url('warehouse/dn_scrap/documentNo'); ?>',
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('warehouse/dn_scrap/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('warehouse/dn_scrap/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('warehouse/dn_scrap/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('warehouse/dn_scrap/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('warehouse/dn_scrap/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
     ...
     function print_note() {
        ...
        if (document_no == "") {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('warehouse/dn_scrap/checkNote') ?>",
                data: {
                ...
                success: function(response) {
                    ...
                    if (response == 'NO') {
                        ...
                        window.open("<?= base_url('warehouse/dn_scrap/print_note/') ?>" + window.btoa(document_no), "_blank");
                    }
                },
                error: function() {
    ...
    function btnDetails(val, row, index) {
        ...
        if (attachment != null && attachment != "") {
            return '<a class="btn btn-primary w-100" target="_blank" href="<?= base_url('assets/image/dn_scrap/') ?>'+row.attachment+'" style="pointer-events: visible; opacity:1;"><i class="fa fa-eye"></i> View</a>';
        } else {
            return '';
        }
    }
