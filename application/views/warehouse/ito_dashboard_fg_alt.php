...
<script>
    ...
    function loadDashboard() {
        ...
        console.log('<?= base_url('warehouse/Ito_dashboard_fg_alt/get_dashboard_datatables_peritem') ?>' + url);

        
        
        $('#dg').datagrid('options').url = '<?= base_url('warehouse/Ito_dashboard_fg_alt/get_dashboard_datatables_peritem') ?>' + url;
    ...
    function loadcharts(data_pie,color,data_bar){
        ...
        const labels = top5Data.map(item => item.id_item);
        ...
        const categories = top5Bar.map(item => item.id_item);
        const stockIn = top5Bar.map(item => parseFloat(item.stock_out).toFixed(2));
