...
<script>
    ...
    function loadDashboard() {
        ...
        console.log('<?= base_url('warehouse/Ito_dashboard_fg/get_dashboard_datatables') ?>' + url);

        
        
        $('#dg').datagrid('options').url = '<?= base_url('warehouse/Ito_dashboard_fg/get_dashboard_datatables') ?>' + url;
    ...
    function loadcharts(data_pie,color,data_bar){
        ...
        const labels = top5Data.map(item => item.customer_name);
        ...
        const categories = top5Bar.map(item => item.customer_name);
