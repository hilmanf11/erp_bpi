...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('warehouse/upload_receipt_fg/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    // UPLOAD DATA
    function upload() {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_upload_receipt_fg.xls') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('warehouse/upload_receipt_fg/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('warehouse/upload_receipt_fg/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('warehouse/upload_receipt_fg/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('warehouse/upload_receipt_fg/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('warehouse/upload_receipt_fg/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
