...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('warehouse/transaction_other_component/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //Add Data
    function add() {
    ...
    function request_no(reqDate = "") {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('warehouse/transaction_other_component/request_no') ?>/" + window.btoa(request_date),
            dataType: "html",
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('warehouse/transaction_other_component/readItemRm') ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
        ...
        if (row) {
            if(row.state == "closed"){
                ...
                $("#transaction_id").combobox({
                    url: '<?= base_url('warehouse/transaction_other_component/readType/') ?>',
                ...
                addTable('<?= base_url('warehouse/transaction_other_component/datatableUpdate/') ?>' + window.btoa(row.request_no));
            }else{
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('warehouse/transaction_other_component/delete') ?>',
                            data: {
                                id: row.id,
    ...
    function filter() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('warehouse/transaction_other_component/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('warehouse/transaction_other_component/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        url = "?&filter_request_no=" + filter_request_no + "&filter_product_no=" + btoa(filter_product_no) + "&filter_product_family=" + filter_product_family + "&filter_from=" + filter_from + "&filter_to=" + filter_to + "&filter_transaction_type=" + filter_transaction_type;
        window.location.assign('<?= base_url('warehouse/transaction_other_component/print/excel') ?>' + url);
    }
    ...
    $(function() {
        addTable();
        $('#dg').treegrid({
            url: '<?= base_url('warehouse/transaction_other_component/datatables') ?>',
            pagination: true,
            rownumbers: true,
            idField: 'id',
                    ...
                    if (totalrows <= 0) {
                        ...
                        for (let i = 0; i < totalrows; i++) {
                            if (rows[i].item_rm_id) {
                                $.ajax({
                                    type: "post",
                                    url: '<?= base_url('warehouse/transaction_other_component/create') ?>',
                                    data: {
        ...
        $("#filter_period").combobox({
            url: '<?= base_url('warehouse/transaction_other_component/readPeriod') ?>',
            valueField: 'period',
            textField: 'period',
            prompt: "Choose period",
            icons: [{
            ...
            onSelect: function(period) {
                $("#filter_workorder").combobox({
                    url: '<?= base_url('warehouse/transaction_other_component/readWp/') ?>' + period.period,
                    ...
                    onSelect: function(wp) {
                        $("#filter_request_no").combobox({
                            url: '<?= base_url('warehouse/transaction_other_component/readRequestNo/') ?>' + period.period + '/' + window.btoa(wp.workorder),
        ...
        $("#filter_request_no").combobox({
            url: '<?= base_url('warehouse/transaction_other_component/readRequestNos/') ?>',
            valueField: 'request_no',
            textField: 'request_no',
            prompt: "Choose Request No",
            icons: [{
        ...
        $("#filter_product_family").combobox({
            ...
            onSelect: function(prodfam){
                $('#filter_product_no').combogrid({
                    url: '<?= base_url('warehouse/transaction_other_component/readProduct/') ?>' + prodfam.id,
                    panelWidth: 420,
                    idField: 'id',
                    textField: 'number',
                    mode: 'remote',
        ...
        $('#filter_product_no').combogrid({
            url: '<?= base_url('warehouse/transaction_other_component/readProducts/') ?>',
            panelWidth: 420,
            idField: 'id',
            textField: 'number',
            mode: 'remote',
    ...
    function BtnPrintLabel(val, row) {
        return '<a style="text-decoration: none; font-weight:bold;" target="_blank" href="<?= base_url('warehouse/transaction_other_component/print_label/') ?>' + window.btoa(row.id) + '"><i class="fa fa-print"></i> Print</a>';
    }
