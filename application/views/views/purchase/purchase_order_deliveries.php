...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'request_no',width:150,halign:'center'">Purchase Request No</th>
            <th rowspan="2" data-options="field:'supplier_name',width:200,halign:'center'">Supplier Name</th>
            <th rowspan="2" data-options="field:'po_date',width:150,halign:'center'">Purchase Order Date</th>
            ...
            <th rowspan="2" data-options="field:'taxes',width:100,halign:'center',align:'right',formatter: numberFormat">Taxes</th>
...
<!-- DIALOG DELIVERY -->
<div id="dlg_delivery" class="easyui-dialog" title="Purchase Delivery Order" data-options="closed: true,modal:true" style="width: 800px; height: 500px; top: 20px; left: 10px;">
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            <div class="fitem" hidden>
                ...
                <input style="width:60%;" name="supplier_id" id="supplier_id" readonly class="easyui-textbox">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Supplier</span>
                <input style="width:60%;" id="supplier_name" readonly class="easyui-textbox">
            </div>
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Purchase Order No</span>
                <input style="width:60%;" name="po_no" id="po_no" readonly class="easyui-textbox">
            </div>
            <div class="fitem" hidden>
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" id="item_rm_number" readonly class="easyui-textbox">
            </div>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('purchase/purchase_order_deliveries/print') ?>" style="width: 100%;" hidden></iframe>

<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        var supplier_id = $("#supplier_id").textbox('getValue');
        var supplier_name = $("#supplier_name").textbox('getValue');
        var po_no = $("#po_no").textbox('getValue');
        var item_rm_id = $("#item_rm_id").textbox('getValue');
        var item_rm_number = $("#item_rm_number").textbox('getValue');

        url_save = '<?= base_url('purchase/purchase_order_deliveries/create') ?>';
        $('#frm_insert').form('clear');

        $("#supplier_id").textbox('setValue', supplier_id);
        $("#supplier_name").textbox('setValue', supplier_name);
        $("#po_no").textbox('setValue', po_no);
        $("#item_rm_id").textbox('setValue', item_rm_id);
        $("#item_rm_number").textbox('setValue', item_rm_number);
    ...
    function btnDelivery(val, row) {
        console.log(row);
        var delivery = "delivery('" + row.supplier_id + "','" + row.supplier_name + "','" + row.po_no + "','" + row.item_rm_id + "','" + row.item_rm_number + "')"; //mengambil id dari customers kemudian di simpan di function details
        return '<a class="btn btn-primary w-100" onClick="' + delivery + '" style="pointer-events: visible; opacity:1;"><i class="fa fa-truck"></i></a>';
    }

    function delivery(supplier_id, supplier_name, po_no, item_rm_id, item_rm_number) {
        $("#dlg_delivery").dialog('open');
        $("#supplier_id").textbox('setValue', supplier_id); // id customer di simpan di textbox supplier_id sekaligus saat add id tersimpan
        $("#supplier_name").textbox('setValue', supplier_name);
        $("#po_no").textbox('setValue', po_no);
        $("#item_rm_id").textbox('setValue', item_rm_id);
        $("#item_rm_number").textbox('setValue', item_rm_number);
        ...
        $('#dg2').datagrid({
            url: '<?= base_url("purchase/purchase_order_deliveries/datatables2/") ?>' + btoa(supplier_id) + '/' + btoa(po_no) + '/' + btoa(item_rm_id)
        });
    }
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('purchase/purchase_order_deliveries/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function filter() {
        ...
        var url = "?filter_from=" + window.btoa(filter_from) +
            ...
            "&filter_supplier_id=" + window.btoa(filter_supplier_id) +
            "&filter_po_no=" + window.btoa(filter_po_no) +
            "&filter_request_no=" + window.btoa(filter_request_no) +
            "&filter_item_rm=" + window.btoa(filter_item_rm);

        $('#dg').datagrid({
            url: '<?= base_url('purchase/purchase_order_deliveries/datatables') ?>' + url,
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('purchase/purchase_order_deliveries/datatableDetails?po_no=') ?>' + window.btoa(row.po_no),
                    singleSelect: true,
                    rownumbers: true,
                    columns: [
                        [{
        ...
        $("#printout").attr('src', '<?= base_url('purchase/purchase_order_deliveries/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('purchase/purchase_order_deliveries/print/excel') ?>' + url);
    }
    ...
    $('#filter_supplier_id').combobox({
        ...
        onSelect: function(supp) {
            ...
            $('#filter_item_rm').combogrid({
                url: '<?= base_url('purchase/purchase_order_deliveries/readProductNo/'); ?>' + window.btoa(supp.id),
                panelWidth: 400,
                idField: 'id',
                textField: 'number',
                mode: 'remote',
    ...
    $("#filter_request_no").combobox({
        url: '<?= base_url('purchase/purchase_order_deliveries/readRequestnumber') ?>',
