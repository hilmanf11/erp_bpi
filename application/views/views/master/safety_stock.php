...
<table id="dg" class="easyui-datagrid" style="width:99.5%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'item_fg_name',width:150,align:'center'">Product Name.</th>
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="item_fg_name" id="item_fg_name" required="" class="easyui-textbox">
            </div>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('master/safety_stock/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('master/safety_stock/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('master/safety_stock/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('master/safety_stock/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_safety_stock.xls') ?>');
    }
    //PRINT PDF
    function pdf() {
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('master/safety_stock/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/safety_stock/datatables') ?>',
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
    ...
    $(document).ready(function () {
        $('#safety_stock, #prod_plan').numberbox({
    ...
    $('#item_fg_id').combogrid({
            url: '<?php echo base_url('master/safety_stock/readItems'); ?>',
            required: true,
            panelWidth: 500,
            idField: 'item_fg_id',
            ...
            onSelect: function(val, rows) {
                $("#item_fg_number").textbox('setValue', rows.item_fg_number);
    ...
    $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('master/safety_stock/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('master/safety_stock/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('master/safety_stock/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('master/safety_stock/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('master/safety_stock/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
