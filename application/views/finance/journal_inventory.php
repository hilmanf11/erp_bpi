    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:70%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;" id="fieldset">
            ...
            <div style="width:50%; float:left;">
                ...
                <div class="fitem">
                    <span style="width:35%; display:inline-block;">Type</span>
                    <select style="width:60%;" id="type" name="type" class="easyui-combobox" required 
...
<script>
            ...
            $.ajax({
                ...
                success: function(result) {
                    if(result == 0){
                        var lastIndex;
                        var dg = $('#dg2').datagrid({
                            url: '<?= base_url('finance/journal_inventory/datatablesTemp') ?>?journal_date=' + window.btoa(journal_date) +
    ...
    function update() {
        ...
        if (row) {
            ...
            var lastIndex;
            $('#dg2').datagrid({
                url: '<?= base_url('finance/journal_inventory/datatableUpdates') ?>?number=' + window.btoa(row.number),
                onLoadSuccess: function(data){
                    $("#local_debit").numberbox('setValue', data['footer'][0].local_debit);
                    $("#local_credit").numberbox('setValue', data['footer'][0].local_credit);
                },
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('finance/journal_inventory/datatables') ?>' + url,
            pagination: true,
            rownumbers: true,
            fit:true,
        });
        ...
        $("#printout").attr('src', '<?= base_url('finance/journal_inventory/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('finance/journal_inventory/print/excel') ?>' + url);
    }
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_journal_inventory.xls') ?>');
    }
    ...
    function print_voucher() {
        ...
        if (row) {
            window.open("<?= base_url('finance/journal_inventory/print_voucher/') ?>" + window.btoa(row.number), "_blank", 'location=yes,height=600,width=1200,scrollbars=yes,status=yes');
        } else {
    ...
    $(function(){
        ...
        // 1. Load data JSON
        $.getJSON('<?php echo base_url("json/journal_inventory_module.json"); ?>', function(data){
            masterModules = data;
            
            // Inisialisasi awal: Tampilkan SEMUA (IN + OUT)
            refreshModuleCombo(''); 
        });
        ...
        function refreshModuleCombo(selectedGroup) {
            ...
            if (selectedGroup === '' || selectedGroup === null) {
                // Jika Type "Choose All", tampilkan semua data dari JSON
                filtered = masterModules;
            } else {
                // Filter array berdasarkan properti 'group'
                filtered = masterModules.filter(function(item){
                    return item.group === selectedGroup;
                });
            }

            // Tambahkan opsi "Choose All" di paling atas hasil filter
            var finalData = [{id: '', text: 'Choose All', group: ''}].concat(filtered);
            
            $('#filter_modul').combobox('loadData', finalData);
            $('#filter_modul').combobox('setValue', ''); 
        }

        // 3. Event Listener pada Dropdown Type
        $('#filter_type').combobox({
            onSelect: function(rec){
                refreshModuleCombo(rec.value); // rec.value berisi 'IN' atau 'OUT'
            },
            onChange: function(newValue){
                if(newValue === '') refreshModuleCombo('');
            }
        });
    });
    ...
    $(function(){
        ...
        // 1. Load data JSON satu kali saat halaman siap
        $.getJSON('<?php echo base_url("json/journal_inventory_module.json"); ?>', function(data){
            masterModules = data;
            
            // Inisialisasi awal: Tampilkan SEMUA modul (IN & OUT)
            refreshModuleCombo(''); 
        });

        /**
         * Fungsi untuk memfilter dan memuat data ke Combobox Modul
         * @param {string} selectedGroup - Nilai 'IN', 'OUT', atau ''
         */
        function refreshModuleCombo(selectedGroup) {
            var filteredData;

            if (selectedGroup === '' || selectedGroup === null) {
                // Jika Type "Choose All", tampilkan seluruh data
                filteredData = masterModules;
            } else {
                // Filter array berdasarkan properti 'group'
                filteredData = masterModules.filter(function(item){
                    return item.group === selectedGroup;
                });
            }

            // Tambahkan opsi default "Choose All" di paling atas hasil filter
            // Kita beri group 'ALL' agar muncul terpisah secara visual
            var finalResult = [{id: '', text: 'Choose All', group: ''}].concat(filteredData);
            
            // Load ke EasyUI Combobox
            $('#modul').combobox('loadData', finalResult);
            
            // Reset pilihan Modul ke default setiap kali Type berubah
            $('#modul').combobox('setValue', ''); 
        }

        // 2. Event Listener pada Dropdown Type
        $('#type').combobox({
            onSelect: function(rec){
                // rec.value akan berisi 'IN', 'OUT', atau ''
                refreshModuleCombo(rec.value);
            },
            onChange: function(newValue){
                // Menangani jika user menghapus teks secara manual
                if(newValue === '') refreshModuleCombo('');
            }
        });
    });
    ...
    $(function() {
                        ...
                        // success: function (lock) {
                            ...
                            if (rows.length > 0) {
                                ...
                                for (var i = 0; i < totalrows; i++) {
                                    if (rows[i].currency !== 'IDR' && (parseFloat(rows[i].rates) === 0 || parseFloat(rows[i].rates) === 1 || rows[i].rates == null || rows[i].rates === '')) {
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('finance/journal_inventory/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('finance/journal_inventory/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('finance/journal_inventory/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('finance/journal_inventory/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('finance/journal_inventory/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
        ...
        $("#filter_journal_type").combogrid({
            url: '<?= base_url('finance/journal_inventory/readJournalType?modul=') ?>',
            panelWidth: 360,
            idField: 'id',
            textField: 'name',
            mode: 'remote',
            ...
            onChange: function(modul) {
                ...
                if (journal_date == "") {
                    ...
                    $("#journal_type").combogrid({
                        url: '<?= base_url('finance/journal_inventory/readJournalType?journal_date=') ?>' + btoa(journal_date) + "&modul=" + btoa(modul),
                        panelWidth: 360,
                        idField: 'id',
                        textField: 'name',
                        mode: 'remote',
                        ...
                        onSelect: function(val, row) {
                            $("#company_name").combobox({
                                url: '<?= base_url('finance/journal_inventory/readCompany?modul=') ?>' + btoa(modul) + "&journal_date=" + btoa(journal_date) + "&journal_type=" + btoa(row.id),
                                valueField: 'company_id',
                                textField: 'company_name',
                                prompt: "Choose Company Name",
                                onSelect: function(rowcom) {
                                    $("#document_no").combogrid({
                                        url: '<?= base_url('finance/journal_inventory/readModul?modul=') ?>' + btoa(modul) + "&journal_date=" + btoa(journal_date) + "&journal_type=" + btoa(row.id) + "&company_id=" + btoa(rowcom.company_id),
                                        panelWidth: 200,
                                        idField: 'number',
                                        textField: 'number',
                                        multiple: true,
