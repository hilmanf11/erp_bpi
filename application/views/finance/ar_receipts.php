...
<script>
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_ar_receipts.xls') ?>');
    }
    ...
    $(function() {
        ...
        $('#dlg_upload').dialog({ 
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('finance/ar_receipts/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
        ...
        function processUpload(total, data, index = 0) {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('finance/ar_receipts/uploadCreate') ?>",
                data: { "data": itemData },
                dataType: "json",
                success: function(response) {
                    let title;
                    if (response.theme === "success") {
                        ...
                        $.post("<?= base_url('finance/ar_receipts/uploadcreateFailed') ?>", {
                            data: itemData,
                ...
                error: function(xhr, status, error) {
                    let salesInvoice = data[index].sales_invoice;
                    let errorMessage = `Failed to upload! Sales Invoice No. ${salesInvoice}. Status: ${status}, Error: ${error}`;
        ...
        function getJournalAndProcess() {
            $.ajax({
                type: "POST",
                url: "<?= base_url('finance/ar_receipts/uploadGetJournal') ?>",
                dataType: "json",
        ...
        function processUpload2(total, data, index = 0) {
            if (index >= total) {
                ...
                // Clear file
                $.get("<?= base_url('finance/ar_receipts/uploadclearFailed') ?>");
                return;
            }
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url('finance/ar_receipts/uploadCreateJournal') ?>",
                data: { "data": itemData },
                dataType: "json",
                success: function(response) {
                    let title;
                ...
                error: function(xhr, status, error) {
                    let receiptNo = data[index].receipt_no;
                    let accountNo = data[index].account_number;
                    let errorMessage = `Failed to upload Journal! Receipt No. ${receiptNo} & Account No. ${accountNo}. Status: ${status}, Error: ${error}`;
                    $("#p_remarks").append(`<b style='color: red;'>Error</b> | ${errorMessage}<br>`);
                    processUpload2(total, data, index + 1);
                }
            });
