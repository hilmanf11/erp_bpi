        ...
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div style="float:left; width: 50%;">
                ...
                <div class="fitem">
                    ...
                    <select style="width:60%;" name="type" id="type" class="easyui-combobox">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('finance/asset_categories/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
    ...
    function append() {
        var number = $("#number").combogrid('getValue');
    ...
    function update() {
        ...
        if (row) {
            if (row.datatable == "1") {
                ...
                $("#number").combogrid('disable');
                $("#name").textbox('disable');
                $("#type").combobox('disable');
                addTable('<?= base_url('finance/asset_categories/datatable_updates?number=') ?>' + window.btoa(row.number));
            } else {
                ...
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if (row.state == "closed") {
                            ...
                            $.ajax({
                                method: 'post',
                                url: '<?= base_url('finance/asset_categories/delete') ?>',
                                data: {
                                    id: row.id
                                },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('finance/asset_categories/print/excel') ?>');
    }
    //RELOAD
    function reload() {
        window.location.reload();
    ...
    $(function() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('finance/asset_categories/datatables') ?>',
            pagination: false,
            rownumbers: true,
            idField: 'id',
            treeField: 'number',
                ...
                handler: function() {
                    ...
                    for (let i = 0; i < totalrows; i++) {
                        if (rows[i].account_number) {
                            $.ajax({
                                type: "post",
                                url: '<?= base_url('finance/asset_categories/create') ?>',
                                data: {
                                    number: number,
                                    name: name,
                                    type: type,
        ...
        $("#number").combogrid({
            url: '<?= base_url('finance/asset_categories/readfamilies') ?>',
            panelWidth: 300,
            idField: 'number',
            textField: 'number',
            mode: 'remote',
